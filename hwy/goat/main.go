...
// runObjdump runs objdump with the given arguments, falling back to
// llvm-objdump if the system objdump fails (e.g. can't read Mach-O on Linux).
func runObjdump(args ...string) (string, error) {
	dump, err := runCommand("objdump", args...)
	if err != nil {
		if _, lookErr := exec.LookPath("llvm-objdump"); lookErr == nil {
			return runCommand("llvm-objdump", args...)
		}
	}
	return dump, err
}

func fetchObjdumpVersion() string {
	v := fetchVersion("objdump")
	if _, err := exec.LookPath("llvm-objdump"); err == nil {
		v += " (llvm-objdump " + fetchVersion("llvm-objdump") + ")"
	}
	return v
}
