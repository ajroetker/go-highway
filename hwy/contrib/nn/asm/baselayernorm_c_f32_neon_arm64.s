//go:build !noasm && arm64
// Code generated by GoAT. DO NOT EDIT.
// versions:
// 	clang   22.1.1
// 	objdump 2.46.0.20260210
// flags: -march=armv8-a+simd+fp -fno-builtin-memset -O3
// source: /Users/ajroetker/go/src/github.com/ajroetker/go-highway/hwy/contrib/nn/asm/baselayernorm_c_f32_neon_arm64.c

TEXT ·layernorm_c_f32_neon(SB), $16-80
	MOVD input+0(FP), R0
	MOVD output+8(FP), R1
	MOVD pnormSize+16(FP), R2
	MOVD gamma+24(FP), R3
	MOVD beta+32(FP), R4
	MOVD pepsilon+40(FP), R5
	MOVD plen_input+48(FP), R6
	MOVD plen_output+56(FP), R7
	MOVD plen_gamma+64(FP), R8
	MOVD R8, 0(RSP)
	MOVD plen_beta+72(FP), R8
	MOVD R8, 8(RSP)
	WORD $0xf94000c9            // ldr	x9, [x6]
	WORD $0xf94000ea            // ldr	x10, [x7]
	WORD $0xf9400048            // ldr	x8, [x2]
	WORD $0xeb0a013f            // cmp	x9, x10
	WORD $0x9a8ab129            // csel	x9, x9, x10, lt
	WORD $0xf100013f            // cmp	x9, #0
	WORD $0xfa411908            // ccmp	x8, #1, #8, ne
	BLT  BB0_36
	WORD $0x9ac80d29            // sdiv	x9, x9, x8
	WORD $0xf100053f            // cmp	x9, #1
	BLT  BB0_36
	WORD $0xd280000a            // mov	x10, #0                         ; =0x0
	WORD $0xbd4000a0            // ldr	s0, [x5]
	WORD $0x9e230101            // ucvtf	s1, x8
	WORD $0x1e2e1002            // fmov	s2, #1.00000000
	WORD $0x1e211841            // fdiv	s1, s2, s1
	WORD $0xf100007f            // cmp	x3, #0
	WORD $0xfa401884            // ccmp	x4, #0, #4, ne
	WORD $0x1a9f07eb            // cset	w11, ne
	WORD $0xd37ef50c            // lsl	x12, x8, #2
	WORD $0x1e6e1002            // fmov	d2, #1.00000000
	B    BB0_4

BB0_3:
	WORD $0x9100054a // add	x10, x10, #1
	WORD $0x8b0c0000 // add	x0, x0, x12
	WORD $0x8b0c0021 // add	x1, x1, x12
	WORD $0xeb09015f // cmp	x10, x9
	BEQ  BB0_36

BB0_4:
	WORD $0xf100111f // cmp	x8, #4
	BHS  BB0_6
	WORD $0xd280000d // mov	x13, #0                         ; =0x0
	WORD $0x6f00e403 // movi.2d	v3, #0000000000000000
	WORD $0x6e23d463 // faddp.4s	v3, v3, v3
	WORD $0x7e30d863 // faddp.2s	s3, v3
	WORD $0xeb0801bf // cmp	x13, x8
	BLT  BB0_9
	B    BB0_10

BB0_6:
	WORD $0x6f00e403 // movi.2d	v3, #0000000000000000
	WORD $0xaa0003ed // mov	x13, x0
	WORD $0x5280008e // mov	w14, #4                         ; =0x4

BB0_7:
	WORD $0x3cc105a4 // ldr	q4, [x13], #16
	WORD $0x4e24d463 // fadd.4s	v3, v3, v4
	WORD $0x910011ce // add	x14, x14, #4
	WORD $0xeb0801df // cmp	x14, x8
	BLE  BB0_7
	WORD $0x927ef10d // and	x13, x8, #0x7ffffffffffffffc
	WORD $0x6e23d463 // faddp.4s	v3, v3, v3
	WORD $0x7e30d863 // faddp.2s	s3, v3
	WORD $0xeb0801bf // cmp	x13, x8
	BGE  BB0_10

BB0_9:
	WORD $0xbc6d7804 // ldr	s4, [x0, x13, lsl #2]
	WORD $0x1e242863 // fadd	s3, s3, s4
	WORD $0x910005ad // add	x13, x13, #1
	WORD $0xeb0d011f // cmp	x8, x13
	BNE  BB0_9

BB0_10:
	WORD $0x1e230823 // fmul	s3, s1, s3
	WORD $0x4e040465 // dup.4s	v5, v3[0]
	WORD $0xf100111f // cmp	x8, #4
	BHS  BB0_12
	WORD $0xd280000d // mov	x13, #0                         ; =0x0
	WORD $0x6f00e404 // movi.2d	v4, #0000000000000000
	WORD $0x6e24d484 // faddp.4s	v4, v4, v4
	WORD $0x7e30d884 // faddp.2s	s4, v4
	WORD $0xeb0801bf // cmp	x13, x8
	BLT  BB0_15
	B    BB0_16

BB0_12:
	WORD $0x6f00e404 // movi.2d	v4, #0000000000000000
	WORD $0xaa0003ed // mov	x13, x0
	WORD $0x5280008e // mov	w14, #4                         ; =0x4

BB0_13:
	WORD $0x3cc105a6 // ldr	q6, [x13], #16
	WORD $0x4ea5d4c6 // fsub.4s	v6, v6, v5
	WORD $0x4e26ccc4 // fmla.4s	v4, v6, v6
	WORD $0x910011ce // add	x14, x14, #4
	WORD $0xeb0801df // cmp	x14, x8
	BLE  BB0_13
	WORD $0x927ef10d // and	x13, x8, #0x7ffffffffffffffc
	WORD $0x6e24d484 // faddp.4s	v4, v4, v4
	WORD $0x7e30d884 // faddp.2s	s4, v4
	WORD $0xeb0801bf // cmp	x13, x8
	BGE  BB0_16

BB0_15:
	WORD $0xbc6d7806 // ldr	s6, [x0, x13, lsl #2]
	WORD $0x1e2338c6 // fsub	s6, s6, s3
	WORD $0x1f0610c4 // fmadd	s4, s6, s6, s4
	WORD $0x910005ad // add	x13, x13, #1
	WORD $0xeb0d011f // cmp	x8, x13
	BNE  BB0_15

BB0_16:
	WORD $0x1e240824 // fmul	s4, s1, s4
	WORD $0x1e242804 // fadd	s4, s0, s4
	WORD $0x1e22c084 // fcvt	d4, s4
	WORD $0x1e61c084 // fsqrt	d4, d4
	WORD $0x1e641844 // fdiv	d4, d2, d4
	WORD $0x1e624084 // fcvt	s4, d4
	WORD $0xf100111f // cmp	x8, #4
	CBZW R11, BB0_19
	BHS  BB0_22
	WORD $0xd280000d // mov	x13, #0                         ; =0x0
	B    BB0_24

BB0_19:
	CBZ  R3, BB0_26
	BHS  BB0_28
	WORD $0xd280000d // mov	x13, #0                         ; =0x0
	B    BB0_30

BB0_22:
	WORD $0xd280000e // mov	x14, #0                         ; =0x0
	WORD $0xd280000d // mov	x13, #0                         ; =0x0

BB0_23:
	WORD $0x3cee6806 // ldr	q6, [x0, x14]
	WORD $0x4ea5d4c6 // fsub.4s	v6, v6, v5
	WORD $0x4f8490c6 // fmul.4s	v6, v6, v4[0]
	WORD $0x3cee6867 // ldr	q7, [x3, x14]
	WORD $0x3cee6890 // ldr	q16, [x4, x14]
	WORD $0x4e26ccf0 // fmla.4s	v16, v7, v6
	WORD $0x3cae6830 // str	q16, [x1, x14]
	WORD $0x910021af // add	x15, x13, #8
	WORD $0x910011ad // add	x13, x13, #4
	WORD $0x910041ce // add	x14, x14, #16
	WORD $0xeb0801ff // cmp	x15, x8
	BLE  BB0_23

BB0_24:
	WORD $0xeb0801bf // cmp	x13, x8
	BGE  BB0_3

BB0_25:
	WORD $0xbc6d7805 // ldr	s5, [x0, x13, lsl #2]
	WORD $0x1e2338a5 // fsub	s5, s5, s3
	WORD $0xbc6d7866 // ldr	s6, [x3, x13, lsl #2]
	WORD $0xbc6d7887 // ldr	s7, [x4, x13, lsl #2]
	WORD $0x1e2408a5 // fmul	s5, s5, s4
	WORD $0x1f061ca5 // fmadd	s5, s5, s6, s7
	WORD $0xbc2d7825 // str	s5, [x1, x13, lsl #2]
	WORD $0x910005ad // add	x13, x13, #1
	WORD $0xeb0d011f // cmp	x8, x13
	BNE  BB0_25
	B    BB0_3

BB0_26:
	BHS  BB0_32
	WORD $0xd280000d // mov	x13, #0                         ; =0x0
	B    BB0_34

BB0_28:
	WORD $0xd280000e // mov	x14, #0                         ; =0x0
	WORD $0xd280000d // mov	x13, #0                         ; =0x0

BB0_29:
	WORD $0x3cee6806 // ldr	q6, [x0, x14]
	WORD $0x4ea5d4c6 // fsub.4s	v6, v6, v5
	WORD $0x4f8490c6 // fmul.4s	v6, v6, v4[0]
	WORD $0x3cee6867 // ldr	q7, [x3, x14]
	WORD $0x6e26dce6 // fmul.4s	v6, v7, v6
	WORD $0x3cae6826 // str	q6, [x1, x14]
	WORD $0x910021af // add	x15, x13, #8
	WORD $0x910011ad // add	x13, x13, #4
	WORD $0x910041ce // add	x14, x14, #16
	WORD $0xeb0801ff // cmp	x15, x8
	BLE  BB0_29

BB0_30:
	WORD $0xeb0801bf // cmp	x13, x8
	BGE  BB0_3

BB0_31:
	WORD $0xbc6d7805 // ldr	s5, [x0, x13, lsl #2]
	WORD $0x1e2338a5 // fsub	s5, s5, s3
	WORD $0xbc6d7866 // ldr	s6, [x3, x13, lsl #2]
	WORD $0x1e2408a5 // fmul	s5, s5, s4
	WORD $0x1e2508c5 // fmul	s5, s6, s5
	WORD $0xbc2d7825 // str	s5, [x1, x13, lsl #2]
	WORD $0x910005ad // add	x13, x13, #1
	WORD $0xeb0d011f // cmp	x8, x13
	BNE  BB0_31
	B    BB0_3

BB0_32:
	WORD $0xd280000e // mov	x14, #0                         ; =0x0
	WORD $0xd280000d // mov	x13, #0                         ; =0x0

BB0_33:
	WORD $0x3cee6806 // ldr	q6, [x0, x14]
	WORD $0x4ea5d4c6 // fsub.4s	v6, v6, v5
	WORD $0x4f8490c6 // fmul.4s	v6, v6, v4[0]
	WORD $0x3cae6826 // str	q6, [x1, x14]
	WORD $0x910021af // add	x15, x13, #8
	WORD $0x910011ad // add	x13, x13, #4
	WORD $0x910041ce // add	x14, x14, #16
	WORD $0xeb0801ff // cmp	x15, x8
	BLE  BB0_33

BB0_34:
	WORD $0xeb0801bf // cmp	x13, x8
	BGE  BB0_3

BB0_35:
	WORD $0xbc6d7805 // ldr	s5, [x0, x13, lsl #2]
	WORD $0x1e2338a5 // fsub	s5, s5, s3
	WORD $0x1e2408a5 // fmul	s5, s5, s4
	WORD $0xbc2d7825 // str	s5, [x1, x13, lsl #2]
	WORD $0x910005ad // add	x13, x13, #1
	WORD $0xeb0d011f // cmp	x8, x13
	BNE  BB0_35
	B    BB0_3

BB0_36:
	RET
