...
// QKVDenseCF16 computes QKVDense using NEON SIMD assembly.
func QKVDenseCF16(x, wQKV, biasQ, biasK, biasV, q, k, v []hwy.Float16, batchSize, inFeatures, qDim, kvDim int) {
...
// QKVDenseCBF16 computes QKVDense using NEON SIMD assembly.
func QKVDenseCBF16(x, wQKV, biasQ, biasK, biasV, q, k, v []hwy.BFloat16, batchSize, inFeatures, qDim, kvDim int) {
	var p_x unsafe.Pointer
	if len(x) > 0 {
		p_x = unsafe.Pointer(&x[0])
	}
...
// QKVDenseCF32 computes QKVDense using NEON SIMD assembly.
func QKVDenseCF32(x, wQKV, biasQ, biasK, biasV, q, k, v []float32, batchSize, inFeatures, qDim, kvDim int) {
	var p_x unsafe.Pointer
	if len(x) > 0 {
		p_x = unsafe.Pointer(&x[0])
	}
...
// QKVDenseCF64 computes QKVDense using NEON SIMD assembly.
func QKVDenseCF64(x, wQKV, biasQ, biasK, biasV, q, k, v []float64, batchSize, inFeatures, qDim, kvDim int) {
	var p_x unsafe.Pointer
	if len(x) > 0 {
		p_x = unsafe.Pointer(&x[0])
	}
