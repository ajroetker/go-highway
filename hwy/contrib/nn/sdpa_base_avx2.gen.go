...
func BaseSDPA_avx2_Float16(q []hwy.Float16, k []hwy.Float16, v []hwy.Float16, mask []hwy.Float16, scores []hwy.Float16, output []hwy.Float16, seqLen int, kvLen int, headDim int, scale hwy.Float16) {
	...
	for i := range seqLen {
		...
		for j := range kvLen {
			vOff := j * headDim
			s := scores[sOff+j]
			vS := asm.BroadcastFloat16x8AVX2(uint16(s))
			d = 0
			for ; d+lanes <= headDim; d += lanes {
				vV := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&v[vOff+d:][0]))
				vO := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&output[oOff+d:][0]))
...
func BaseSDPA_avx2_BFloat16(q []hwy.BFloat16, k []hwy.BFloat16, v []hwy.BFloat16, mask []hwy.BFloat16, scores []hwy.BFloat16, output []hwy.BFloat16, seqLen int, kvLen int, headDim int, scale hwy.BFloat16) {
	...
	for i := range seqLen {
		...
		for j := range kvLen {
			vOff := j * headDim
			s := scores[sOff+j]
			vS := asm.BroadcastBFloat16x8AVX2(uint16(s))
			d = 0
			for ; d+lanes <= headDim; d += lanes {
				vV := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&v[vOff+d:][0]))
				vO := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&output[oOff+d:][0]))
				vS.MulAdd(vV, vO).StorePtr(unsafe.Pointer(&output[oOff+d:][0]))
			}
			for ; d < headDim; d++ {
...
func BaseSDPA_avx2(q []float32, k []float32, v []float32, mask []float32, scores []float32, output []float32, seqLen int, kvLen int, headDim int, scale float32) {
	...
	for i := range seqLen {
		...
		for ; d+lanes <= headDim; d += lanes {
			vZero.Store((*[8]float32)(unsafe.Pointer(&output[oOff+d])))
		}
		for ; d < headDim; d++ {
			output[oOff+d] = 0
		}
		for j := range kvLen {
			vOff := j * headDim
			s := scores[sOff+j]
			vS := archsimd.BroadcastFloat32x8(s)
			d = 0
			for ; d+lanes <= headDim; d += lanes {
				vV := archsimd.LoadFloat32x8((*[8]float32)(unsafe.Pointer(&v[vOff+d])))
				vO := archsimd.LoadFloat32x8((*[8]float32)(unsafe.Pointer(&output[oOff+d])))
				vS.MulAdd(vV, vO).Store((*[8]float32)(unsafe.Pointer(&output[oOff+d])))
			}
			for ; d < headDim; d++ {
...
func BaseSDPA_avx2_Float64(q []float64, k []float64, v []float64, mask []float64, scores []float64, output []float64, seqLen int, kvLen int, headDim int, scale float64) {
	...
	for i := range seqLen {
		...
		for ; d+lanes <= headDim; d += lanes {
			vZero.Store((*[4]float64)(unsafe.Pointer(&output[oOff+d])))
		}
		for ; d < headDim; d++ {
			output[oOff+d] = 0
		}
		for j := range kvLen {
			vOff := j * headDim
			s := scores[sOff+j]
			vS := archsimd.BroadcastFloat64x4(s)
			d = 0
			for ; d+lanes <= headDim; d += lanes {
				vV := archsimd.LoadFloat64x4((*[4]float64)(unsafe.Pointer(&v[vOff+d])))
				vO := archsimd.LoadFloat64x4((*[4]float64)(unsafe.Pointer(&output[oOff+d])))
				vS.MulAdd(vV, vO).Store((*[4]float64)(unsafe.Pointer(&output[oOff+d])))
			}
			for ; d < headDim; d++ {
				output[oOff+d] += s * v[vOff+d]
			}
