...
func BaseLayerNorm_avx2_Float16(input []hwy.Float16, output []hwy.Float16, normSize int, gamma []hwy.Float16, beta []hwy.Float16, epsilon hwy.Float16) {
	...
	for g := range numGroups {
		...
		for ; ii+lanes <= normSize; ii += lanes {
			x := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&input[off+ii]))
			sumAcc = sumAcc.Add(x)
		}
		...
		if gamma != nil && beta != nil {
			...
			for ; ii+lanes <= normSize; ii += lanes {
				...
				g := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&gamma[ii]))
				b := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&beta[ii]))
...
func BaseLayerNorm_avx2_BFloat16(input []hwy.BFloat16, output []hwy.BFloat16, normSize int, gamma []hwy.BFloat16, beta []hwy.BFloat16, epsilon hwy.BFloat16) {
	...
	for g := range numGroups {
		...
		ii := 0
		for ; ii+lanes <= normSize; ii += lanes {
			x := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&input[off+ii]))
			sumAcc = sumAcc.Add(x)
		}
		mean := sumAcc.ReduceSum()
		for i := ii; i < normSize; i++ {
		...
		if gamma != nil && beta != nil {
			...
			for ; ii+lanes <= normSize; ii += lanes {
				...
				diff := x.Sub(vMean)
				normed := diff.Mul(vInvStd)
				g := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&gamma[ii]))
				b := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&beta[ii]))
				result := normed.MulAdd(g, b)
				result.StorePtr(unsafe.Pointer(&output[off+ii]))
			}
			for i := ii; i < normSize; i++ {
...
func BaseLayerNorm_avx2(input []float32, output []float32, normSize int, gamma []float32, beta []float32, epsilon float32) {
	...
	for g := range numGroups {
		...
		ii := 0
		for ; ii+lanes <= normSize; ii += lanes {
			x := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&input[off+ii])))
			sumAcc = sumAcc.Add(x)
		}
		...
		if gamma != nil && beta != nil {
			...
			for ; ii+lanes <= normSize; ii += lanes {
				...
				diff := x.Sub(vMean)
				normed := diff.Mul(vInvStd)
				g := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gamma[ii])))
				b := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&beta[ii])))
				result := normed.MulAdd(g, b)
				result.StoreArray((*[8]float32)(unsafe.Pointer(&output[off+ii])))
			}
			for i := ii; i < normSize; i++ {
...
func BaseLayerNorm_avx2_Float64(input []float64, output []float64, normSize int, gamma []float64, beta []float64, epsilon float64) {
	...
	for g := range numGroups {
		...
		ii := 0
		for ; ii+lanes <= normSize; ii += lanes {
			x := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&input[off+ii])))
			sumAcc = sumAcc.Add(x)
		}
		...
		if gamma != nil && beta != nil {
			...
			for ; ii+lanes <= normSize; ii += lanes {
				...
				diff := x.Sub(vMean)
				normed := diff.Mul(vInvStd)
				g := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&gamma[ii])))
				b := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&beta[ii])))
				result := normed.MulAdd(g, b)
				result.StoreArray((*[4]float64)(unsafe.Pointer(&output[off+ii])))
			}
			for i := ii; i < normSize; i++ {
				normed := (input[off+i] - mean) * invStd
				output[off+i] = normed*gamma[i] + beta[i]
