...
var (
	BaseQuantizeQ8_K_AVX2_maxVec_f32 archsimd.Float32x8
	BaseQuantizeQ8_K_AVX2_minVec_f32 archsimd.Float32x8
	_quantizeKquantBaseAVX2HoistOnce sync.Once
)

func _quantizeKquantBaseAVX2InitHoistedConstants() {
	_quantizeKquantBaseAVX2HoistOnce.Do(func() {
		BaseQuantizeQ8_K_AVX2_maxVec_f32 = archsimd.BroadcastFloat32x8(127.0)
		BaseQuantizeQ8_K_AVX2_minVec_f32 = archsimd.BroadcastFloat32x8(-128.0)
	})
}

func BaseQuantizeQ8_K_avx2(input []float32, output []uint8) {
	_quantizeKquantBaseAVX2InitHoistedConstants()
	if len(input) == 0 {
		return
	}
	...
	lanes := 8
	buf := [8]float32{}
	minVec := BaseQuantizeQ8_K_AVX2_minVec_f32
	maxVec := BaseQuantizeQ8_K_AVX2_maxVec_f32
	for b := range nblocks {
		inOff := b * QK_K
