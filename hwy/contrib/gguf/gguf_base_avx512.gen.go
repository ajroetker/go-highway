...
func BaseDequantizeIQ4NL_avx512(data []uint8, output []float32) {
	...
	for b := range nblocks {
		...
		for ; i+lanes <= 16; i += lanes {
			...
			result.StoreArray((*[16]float32)(unsafe.Pointer(&output[outOff+i])))
		}
		for ; i < 16; i++ {
			lo := qs[i] & 0x0F
		...
		for ; i+lanes <= 16; i += lanes {
			...
			v := archsimd.LoadFloat32x16Array((*[16]float32)(unsafe.Pointer(&buf[0])))
			result := v.Mul(scaleVec)
			result.StoreArray((*[16]float32)(unsafe.Pointer(&output[outOff+16+i])))
		}
		for ; i < 16; i++ {
			hi := (qs[i] >> 4) & 0x0F
	...
	for b := range nblocks {
		...
		for is := range 16 {
			...
			for ; i+lanes <= 16; i += lanes {
				...
				result.StoreArray((*[16]float32)(unsafe.Pointer(&output[baseOut+i])))
			}
			for ; i < 16; i++ {
	...
	for b := range nblocks {
		...
		for chunk := range 4 {
			...
			for ; i+lanes <= 32; i += lanes {
				...
				result.StoreArray((*[16]float32)(unsafe.Pointer(&output[outIdx+i])))
			}
			for ; i < 32; i++ {
			...
			for ; i+lanes <= 32; i += lanes {
				...
				result.StoreArray((*[16]float32)(unsafe.Pointer(&output[outIdx+32+i])))
			}
			for ; i < 32; i++ {
