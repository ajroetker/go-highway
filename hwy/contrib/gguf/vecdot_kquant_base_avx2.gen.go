	...
	for b := range nblocks {
		...
		for is := range 16 {
			...
			for ; i+lanes <= 16; i += lanes {
				...
				wVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&wbuf[0])))
				aVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&abuf[0])))
				accVec = wVec.MulAdd(aVec, accVec)
			}
			accVec.StoreArray((*[8]float32)(unsafe.Pointer(&wbuf[0])))
			var subSum float32
			for k := range lanes {
				subSum += wbuf[k]
			}
