...
func BaseDequantizeQ2K_avx2(data []uint8, output []float32) {
	if len(data) == 0 {
		return
	}
	...
	for b := range nblocks {
		...
		for is := 0; is < 16; is++ {
			...
			dsc := d * sc
			dmm := dmin * m
			dscVec := archsimd.BroadcastFloat32x8(dsc)
			dmmVec := archsimd.BroadcastFloat32x8(dmm)
...
func BaseDequantizeQ3K_avx2(data []uint8, output []float32) {
	if len(data) == 0 {
		return
	}
...
func BaseDequantizeQ4K_avx2(data []uint8, output []float32) {
	if len(data) == 0 {
		return
	}
	...
	for b := range nblocks {
		...
		for chunk := 0; chunk < 4; chunk++ {
			...
			dsc1 := d * scs[is+1]
			dmm1 := dmin * mns[is+1]
			dscVec0 := archsimd.BroadcastFloat32x8(dsc0)
			dmmVec0 := archsimd.BroadcastFloat32x8(dmm0)
			dscVec1 := archsimd.BroadcastFloat32x8(dsc1)
			dmmVec1 := archsimd.BroadcastFloat32x8(dmm1)
			i := 0
...
func BaseDequantizeQ5K_avx2(data []uint8, output []float32) {
	if len(data) == 0 {
		return
	}
...
func BaseDequantizeQ6K_avx2(data []uint8, output []float32) {
	if len(data) == 0 {
		return
	}
