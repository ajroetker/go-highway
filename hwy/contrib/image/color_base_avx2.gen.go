...
func BaseForwardICT_avx2(r *Image[float32], g *Image[float32], b *Image[float32], outY *Image[float32], outCb *Image[float32], outCr *Image[float32]) {
	...
	for row := range height {
		...
		i := 0
		for ; i+lanes <= width; i += lanes {
			vr := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&rRow[i])))
			vg := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gRow[i])))
			vb := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bRow[i])))
			vy := vr.MulAdd(rToYVec, vg.MulAdd(gToYVec, vb.Mul(bToYVec)))
			vcb := vr.MulAdd(rToCbVec, vg.MulAdd(gToCbVec, vb.Mul(bToCbVec)))
			vcr := vr.MulAdd(rToCrVec, vg.MulAdd(gToCrVec, vb.Mul(bToCrVec)))
			vy.StoreArray((*[8]float32)(unsafe.Pointer(&yRow[i])))
			vcb.StoreArray((*[8]float32)(unsafe.Pointer(&cbRow[i])))
			vcr.StoreArray((*[8]float32)(unsafe.Pointer(&crRow[i])))
		}
		if remaining := width - i; remaining > 0 {
			bufR := [8]float32{}
			bufG := [8]float32{}
			...
			copy(bufG[:], gRow[i:i+remaining])
			copy(bufB[:], bRow[i:i+remaining])
			vr := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bufR[0])))
			vg := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bufG[0])))
			vb := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bufB[0])))
			vy := vr.MulAdd(rToYVec, vg.MulAdd(gToYVec, vb.Mul(bToYVec)))
			vcb := vr.MulAdd(rToCbVec, vg.MulAdd(gToCbVec, vb.Mul(bToCbVec)))
			vcr := vr.MulAdd(rToCrVec, vg.MulAdd(gToCrVec, vb.Mul(bToCrVec)))
			vy.StoreArray((*[8]float32)(unsafe.Pointer(&bufY[0])))
			vcb.StoreArray((*[8]float32)(unsafe.Pointer(&bufCb[0])))
			vcr.StoreArray((*[8]float32)(unsafe.Pointer(&bufCr[0])))
			copy(yRow[i:i+remaining], bufY[:remaining])
			copy(cbRow[i:i+remaining], bufCb[:remaining])
			copy(crRow[i:i+remaining], bufCr[:remaining])
		}
...
func BaseForwardICT_avx2_Float64(r *Image[float64], g *Image[float64], b *Image[float64], outY *Image[float64], outCb *Image[float64], outCr *Image[float64]) {
	...
	for row := range height {
		...
		i := 0
		for ; i+lanes <= width; i += lanes {
			vr := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&rRow[i])))
			vg := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&gRow[i])))
			vb := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&bRow[i])))
			vy := vr.MulAdd(rToYVec, vg.MulAdd(gToYVec, vb.Mul(bToYVec)))
			vcb := vr.MulAdd(rToCbVec, vg.MulAdd(gToCbVec, vb.Mul(bToCbVec)))
			vcr := vr.MulAdd(rToCrVec, vg.MulAdd(gToCrVec, vb.Mul(bToCrVec)))
			vy.StoreArray((*[4]float64)(unsafe.Pointer(&yRow[i])))
			vcb.StoreArray((*[4]float64)(unsafe.Pointer(&cbRow[i])))
			vcr.StoreArray((*[4]float64)(unsafe.Pointer(&crRow[i])))
		}
		if remaining := width - i; remaining > 0 {
			bufR := [4]float64{}
			bufG := [4]float64{}
			...
			copy(bufG[:], gRow[i:i+remaining])
			copy(bufB[:], bRow[i:i+remaining])
			vr := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&bufR[0])))
			vg := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&bufG[0])))
			vb := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&bufB[0])))
			vy := vr.MulAdd(rToYVec, vg.MulAdd(gToYVec, vb.Mul(bToYVec)))
			vcb := vr.MulAdd(rToCbVec, vg.MulAdd(gToCbVec, vb.Mul(bToCbVec)))
			vcr := vr.MulAdd(rToCrVec, vg.MulAdd(gToCrVec, vb.Mul(bToCrVec)))
			vy.StoreArray((*[4]float64)(unsafe.Pointer(&bufY[0])))
			vcb.StoreArray((*[4]float64)(unsafe.Pointer(&bufCb[0])))
			vcr.StoreArray((*[4]float64)(unsafe.Pointer(&bufCr[0])))
			copy(yRow[i:i+remaining], bufY[:remaining])
			copy(cbRow[i:i+remaining], bufCb[:remaining])
			copy(crRow[i:i+remaining], bufCr[:remaining])
		}
...
func BaseForwardRCT_avx2_Int32(r *Image[int32], g *Image[int32], b *Image[int32], outY *Image[int32], outCb *Image[int32], outCr *Image[int32]) {
	...
	for y := range height {
		...
		i := 0
		for ; i+lanes <= width; i += lanes {
			vr := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&rRow[i])))
			vg := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&gRow[i])))
			vb := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&bRow[i])))
			twoG := vg.Mul(twoVec)
			sum := vr.Add(twoG).Add(vb)
			vy := sum.ShiftAllRight(uint64(2))
			vcb := vb.Sub(vg)
			vcr := vr.Sub(vg)
			vy.StoreArray((*[8]int32)(unsafe.Pointer(&yRow[i])))
			vcb.StoreArray((*[8]int32)(unsafe.Pointer(&cbRow[i])))
			vcr.StoreArray((*[8]int32)(unsafe.Pointer(&crRow[i])))
		}
		if remaining := width - i; remaining > 0 {
			bufR := [8]int32{}
			bufG := [8]int32{}
			...
			copy(bufG[:], gRow[i:i+remaining])
			copy(bufB[:], bRow[i:i+remaining])
			vr := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&bufR[0])))
			vg := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&bufG[0])))
			vb := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&bufB[0])))
			twoG := vg.Mul(twoVec)
			sum := vr.Add(twoG).Add(vb)
			vy := sum.ShiftAllRight(uint64(2))
			vcb := vb.Sub(vg)
			vcr := vr.Sub(vg)
			vy.StoreArray((*[8]int32)(unsafe.Pointer(&bufY[0])))
			vcb.StoreArray((*[8]int32)(unsafe.Pointer(&bufCb[0])))
			vcr.StoreArray((*[8]int32)(unsafe.Pointer(&bufCr[0])))
			copy(yRow[i:i+remaining], bufY[:remaining])
			copy(cbRow[i:i+remaining], bufCb[:remaining])
			copy(crRow[i:i+remaining], bufCr[:remaining])
		}
...
func BaseForwardRCT_avx2_Int64(r *Image[int64], g *Image[int64], b *Image[int64], outY *Image[int64], outCb *Image[int64], outCr *Image[int64]) {
	...
	for y := range height {
		...
		i := 0
		for ; i+lanes <= width; i += lanes {
			vr := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&rRow[i])))
			vg := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&gRow[i])))
			vb := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&bRow[i])))
			twoG := hwy.Mul_AVX2_Int64x4(vg, twoVec)
			sum := vr.Add(twoG).Add(vb)
			vy := hwy.ShiftAllRight_AVX2_Int64x4(sum, uint64(2))
			vcb := vb.Sub(vg)
			vcr := vr.Sub(vg)
			vy.StoreArray((*[4]int64)(unsafe.Pointer(&yRow[i])))
			vcb.StoreArray((*[4]int64)(unsafe.Pointer(&cbRow[i])))
			vcr.StoreArray((*[4]int64)(unsafe.Pointer(&crRow[i])))
		}
		if remaining := width - i; remaining > 0 {
			bufR := [4]int64{}
			bufG := [4]int64{}
			...
			copy(bufG[:], gRow[i:i+remaining])
			copy(bufB[:], bRow[i:i+remaining])
			vr := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&bufR[0])))
			vg := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&bufG[0])))
			vb := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&bufB[0])))
			twoG := hwy.Mul_AVX2_Int64x4(vg, twoVec)
			sum := vr.Add(twoG).Add(vb)
			vy := hwy.ShiftAllRight_AVX2_Int64x4(sum, uint64(2))
			vcb := vb.Sub(vg)
			vcr := vr.Sub(vg)
			vy.StoreArray((*[4]int64)(unsafe.Pointer(&bufY[0])))
			vcb.StoreArray((*[4]int64)(unsafe.Pointer(&bufCb[0])))
			vcr.StoreArray((*[4]int64)(unsafe.Pointer(&bufCr[0])))
			copy(yRow[i:i+remaining], bufY[:remaining])
			copy(cbRow[i:i+remaining], bufCb[:remaining])
			copy(crRow[i:i+remaining], bufCr[:remaining])
		}
...
func BaseInverseICT_avx2(y *Image[float32], cb *Image[float32], cr *Image[float32], outR *Image[float32], outG *Image[float32], outB *Image[float32]) {
	...
	for row := range height {
		...
		i := 0
		for ; i+lanes <= width; i += lanes {
			vy := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&yRow[i])))
			vcb := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&cbRow[i])))
			vcr := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&crRow[i])))
			vr := vcr.MulAdd(crToRVec, vy)
			vg := vcb.MulAdd(cbToGVec, vcr.MulAdd(crToGVec, vy))
			vb := vcb.MulAdd(cbToBVec, vy)
			vr.StoreArray((*[8]float32)(unsafe.Pointer(&rRow[i])))
			vg.StoreArray((*[8]float32)(unsafe.Pointer(&gRow[i])))
			vb.StoreArray((*[8]float32)(unsafe.Pointer(&bRow[i])))
		}
		if remaining := width - i; remaining > 0 {
			bufY := [8]float32{}
			bufCb := [8]float32{}
			...
			copy(bufCb[:], cbRow[i:i+remaining])
			copy(bufCr[:], crRow[i:i+remaining])
			vy := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bufY[0])))
			vcb := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bufCb[0])))
			vcr := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bufCr[0])))
			vr := vcr.MulAdd(crToRVec, vy)
			vg := vcb.MulAdd(cbToGVec, vcr.MulAdd(crToGVec, vy))
			vb := vcb.MulAdd(cbToBVec, vy)
			vr.StoreArray((*[8]float32)(unsafe.Pointer(&bufR[0])))
			vg.StoreArray((*[8]float32)(unsafe.Pointer(&bufG[0])))
			vb.StoreArray((*[8]float32)(unsafe.Pointer(&bufB[0])))
			copy(rRow[i:i+remaining], bufR[:remaining])
			copy(gRow[i:i+remaining], bufG[:remaining])
			copy(bRow[i:i+remaining], bufB[:remaining])
		}
...
func BaseInverseICT_avx2_Float64(y *Image[float64], cb *Image[float64], cr *Image[float64], outR *Image[float64], outG *Image[float64], outB *Image[float64]) {
	...
	for row := range height {
		...
		i := 0
		for ; i+lanes <= width; i += lanes {
			vy := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&yRow[i])))
			vcb := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&cbRow[i])))
			vcr := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&crRow[i])))
			vr := vcr.MulAdd(crToRVec, vy)
			vg := vcb.MulAdd(cbToGVec, vcr.MulAdd(crToGVec, vy))
			vb := vcb.MulAdd(cbToBVec, vy)
			vr.StoreArray((*[4]float64)(unsafe.Pointer(&rRow[i])))
			vg.StoreArray((*[4]float64)(unsafe.Pointer(&gRow[i])))
			vb.StoreArray((*[4]float64)(unsafe.Pointer(&bRow[i])))
		}
		if remaining := width - i; remaining > 0 {
			bufY := [4]float64{}
			bufCb := [4]float64{}
			...
			copy(bufCb[:], cbRow[i:i+remaining])
			copy(bufCr[:], crRow[i:i+remaining])
			vy := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&bufY[0])))
			vcb := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&bufCb[0])))
			vcr := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&bufCr[0])))
			vr := vcr.MulAdd(crToRVec, vy)
			vg := vcb.MulAdd(cbToGVec, vcr.MulAdd(crToGVec, vy))
			vb := vcb.MulAdd(cbToBVec, vy)
			vr.StoreArray((*[4]float64)(unsafe.Pointer(&bufR[0])))
			vg.StoreArray((*[4]float64)(unsafe.Pointer(&bufG[0])))
			vb.StoreArray((*[4]float64)(unsafe.Pointer(&bufB[0])))
			copy(rRow[i:i+remaining], bufR[:remaining])
			copy(gRow[i:i+remaining], bufG[:remaining])
			copy(bRow[i:i+remaining], bufB[:remaining])
		}
...
func BaseInverseRCT_avx2_Int32(y *Image[int32], cb *Image[int32], cr *Image[int32], outR *Image[int32], outG *Image[int32], outB *Image[int32]) {
	...
	for row := range height {
		...
		i := 0
		for ; i+lanes <= width; i += lanes {
			vy := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&yRow[i])))
			vcb := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&cbRow[i])))
			vcr := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&crRow[i])))
			cbPlusCr := vcb.Add(vcr)
			shift := cbPlusCr.ShiftAllRight(uint64(2))
			vg := vy.Sub(shift)
			vr := vcr.Add(vg)
			vb := vcb.Add(vg)
			vr.StoreArray((*[8]int32)(unsafe.Pointer(&rRow[i])))
			vg.StoreArray((*[8]int32)(unsafe.Pointer(&gRow[i])))
			vb.StoreArray((*[8]int32)(unsafe.Pointer(&bRow[i])))
		}
		if remaining := width - i; remaining > 0 {
			bufY := [8]int32{}
			bufCb := [8]int32{}
			...
			copy(bufCb[:], cbRow[i:i+remaining])
			copy(bufCr[:], crRow[i:i+remaining])
			vy := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&bufY[0])))
			vcb := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&bufCb[0])))
			vcr := archsimd.LoadInt32x8Array((*[8]int32)(unsafe.Pointer(&bufCr[0])))
			cbPlusCr := vcb.Add(vcr)
			shift := cbPlusCr.ShiftAllRight(uint64(2))
			vg := vy.Sub(shift)
			vr := vcr.Add(vg)
			vb := vcb.Add(vg)
			vr.StoreArray((*[8]int32)(unsafe.Pointer(&bufR[0])))
			vg.StoreArray((*[8]int32)(unsafe.Pointer(&bufG[0])))
			vb.StoreArray((*[8]int32)(unsafe.Pointer(&bufB[0])))
			copy(rRow[i:i+remaining], bufR[:remaining])
			copy(gRow[i:i+remaining], bufG[:remaining])
			copy(bRow[i:i+remaining], bufB[:remaining])
		}
...
func BaseInverseRCT_avx2_Int64(y *Image[int64], cb *Image[int64], cr *Image[int64], outR *Image[int64], outG *Image[int64], outB *Image[int64]) {
	...
	for row := range height {
		...
		i := 0
		for ; i+lanes <= width; i += lanes {
			vy := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&yRow[i])))
			vcb := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&cbRow[i])))
			vcr := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&crRow[i])))
			cbPlusCr := vcb.Add(vcr)
			shift := hwy.ShiftAllRight_AVX2_Int64x4(cbPlusCr, uint64(2))
			vg := vy.Sub(shift)
			vr := vcr.Add(vg)
			vb := vcb.Add(vg)
			vr.StoreArray((*[4]int64)(unsafe.Pointer(&rRow[i])))
			vg.StoreArray((*[4]int64)(unsafe.Pointer(&gRow[i])))
			vb.StoreArray((*[4]int64)(unsafe.Pointer(&bRow[i])))
		}
		if remaining := width - i; remaining > 0 {
			bufY := [4]int64{}
			bufCb := [4]int64{}
			...
			copy(bufCb[:], cbRow[i:i+remaining])
			copy(bufCr[:], crRow[i:i+remaining])
			vy := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&bufY[0])))
			vcb := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&bufCb[0])))
			vcr := archsimd.LoadInt64x4Array((*[4]int64)(unsafe.Pointer(&bufCr[0])))
			cbPlusCr := vcb.Add(vcr)
			shift := hwy.ShiftAllRight_AVX2_Int64x4(cbPlusCr, uint64(2))
			vg := vy.Sub(shift)
			vr := vcr.Add(vg)
			vb := vcb.Add(vg)
			vr.StoreArray((*[4]int64)(unsafe.Pointer(&bufR[0])))
			vg.StoreArray((*[4]int64)(unsafe.Pointer(&bufG[0])))
			vb.StoreArray((*[4]int64)(unsafe.Pointer(&bufB[0])))
			copy(rRow[i:i+remaining], bufR[:remaining])
			copy(gRow[i:i+remaining], bufG[:remaining])
			copy(bRow[i:i+remaining], bufB[:remaining])
		}
