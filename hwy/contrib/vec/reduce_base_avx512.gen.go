// Code generated by github.com/ajroetker/go-highway/cmd/hwygen. DO NOT EDIT.

//go:build amd64 && goexperiment.simd

package vec

import (
	"github.com/ajroetker/go-highway/hwy"
	"simd/archsimd"
)

func BaseSum_avx512_Float16(v []hwy.Float16) hwy.Float16 {
	if len(v) == 0 {
		return 0
	}
	sum := hwy.Zero[hwy.Float16]()
	lanes := 32
	var i int
	for i = 0; i+lanes*3 <= len(v); i += lanes * 3 {
		va := hwy.Load(v[i:])
		sum = hwy.AddF16(sum, va)
		va1 := hwy.Load(v[i+32:])
		sum = hwy.AddF16(sum, va1)
		va2 := hwy.Load(v[i+64:])
		sum = hwy.AddF16(sum, va2)
	}
	result := hwy.ReduceSumF16(sum)
	for ; i < len(v); i++ {
		result += v[i].Float32()
	}
	return hwy.Float32ToFloat16(result)
}

func BaseSum_avx512_BFloat16(v []hwy.BFloat16) hwy.BFloat16 {
	if len(v) == 0 {
		return 0
	}
	sum := hwy.Zero[hwy.BFloat16]()
	lanes := 32
	var i int
	for i = 0; i+lanes*3 <= len(v); i += lanes * 3 {
		va := hwy.Load(v[i:])
		sum = hwy.AddBF16(sum, va)
		va1 := hwy.Load(v[i+32:])
		sum = hwy.AddBF16(sum, va1)
		va2 := hwy.Load(v[i+64:])
		sum = hwy.AddBF16(sum, va2)
	}
	result := hwy.ReduceSumBF16(sum)
	for ; i < len(v); i++ {
		result += v[i].Float32()
	}
	return hwy.Float32ToBFloat16(result)
}

func BaseSum_avx512(v []float32) float32 {
	if len(v) == 0 {
		return 0
	}
	sum := archsimd.BroadcastFloat32x16(0)
	lanes := 16
	var i int
	for i = 0; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadFloat32x16Slice(v[i:])
		sum = sum.Add(va)
		va1 := archsimd.LoadFloat32x16Slice(v[i+16:])
		sum = sum.Add(va1)
		va2 := archsimd.LoadFloat32x16Slice(v[i+32:])
		sum = sum.Add(va2)
	}
	result := hwy.ReduceSum_AVX512_F32x16(sum)
	for ; i < len(v); i++ {
		result += v[i]
	}
	return result
}

func BaseSum_avx512_Float64(v []float64) float64 {
	if len(v) == 0 {
		return 0
	}
	sum := archsimd.BroadcastFloat64x8(0)
	lanes := 8
	var i int
	for i = 0; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadFloat64x8Slice(v[i:])
		sum = sum.Add(va)
		va1 := archsimd.LoadFloat64x8Slice(v[i+8:])
		sum = sum.Add(va1)
		va2 := archsimd.LoadFloat64x8Slice(v[i+16:])
		sum = sum.Add(va2)
	}
	result := hwy.ReduceSum_AVX512_F64x8(sum)
	for ; i < len(v); i++ {
		result += v[i]
	}
	return result
}

func BaseMin_avx512_Float16(v []hwy.Float16) hwy.Float16 {
	if len(v) == 0 {
		panic("vec: Min called on empty slice")
	}
	lanes := 32
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i].Float32() < result.Float32() {
				result = v[i]
			}
		}
		return result
	}
	minVec := hwy.Load(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := hwy.Load(v[i:])
		minVec = hwy.MinF16(minVec, va)
		va1 := hwy.Load(v[i+32:])
		minVec = hwy.MinF16(minVec, va1)
		va2 := hwy.Load(v[i+64:])
		minVec = hwy.MinF16(minVec, va2)
	}
	result := hwy.ReduceMinF16(minVec)
	for ; i < len(v); i++ {
		if v[i].Float32() < result.Float32() {
			result = v[i]
		}
	}
	return result
}

func BaseMin_avx512_BFloat16(v []hwy.BFloat16) hwy.BFloat16 {
	if len(v) == 0 {
		panic("vec: Min called on empty slice")
	}
	lanes := 32
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i].Float32() < result.Float32() {
				result = v[i]
			}
		}
		return result
	}
	minVec := hwy.Load(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := hwy.Load(v[i:])
		minVec = hwy.MinBF16(minVec, va)
		va1 := hwy.Load(v[i+32:])
		minVec = hwy.MinBF16(minVec, va1)
		va2 := hwy.Load(v[i+64:])
		minVec = hwy.MinBF16(minVec, va2)
	}
	result := hwy.ReduceMinBF16(minVec)
	for ; i < len(v); i++ {
		if v[i].Float32() < result.Float32() {
			result = v[i]
		}
	}
	return result
}

func BaseMin_avx512(v []float32) float32 {
	if len(v) == 0 {
		panic("vec: Min called on empty slice")
	}
	lanes := 16
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i] < result {
				result = v[i]
			}
		}
		return result
	}
	minVec := archsimd.LoadFloat32x16Slice(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadFloat32x16Slice(v[i:])
		minVec = minVec.Min(va)
		va1 := archsimd.LoadFloat32x16Slice(v[i+16:])
		minVec = minVec.Min(va1)
		va2 := archsimd.LoadFloat32x16Slice(v[i+32:])
		minVec = minVec.Min(va2)
	}
	result := hwy.ReduceMin_AVX512_F32x16(minVec)
	for ; i < len(v); i++ {
		if v[i] < result {
			result = v[i]
		}
	}
	return result
}

func BaseMin_avx512_Float64(v []float64) float64 {
	if len(v) == 0 {
		panic("vec: Min called on empty slice")
	}
	lanes := 8
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i] < result {
				result = v[i]
			}
		}
		return result
	}
	minVec := archsimd.LoadFloat64x8Slice(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadFloat64x8Slice(v[i:])
		minVec = minVec.Min(va)
		va1 := archsimd.LoadFloat64x8Slice(v[i+8:])
		minVec = minVec.Min(va1)
		va2 := archsimd.LoadFloat64x8Slice(v[i+16:])
		minVec = minVec.Min(va2)
	}
	result := hwy.ReduceMin_AVX512_F64x8(minVec)
	for ; i < len(v); i++ {
		if v[i] < result {
			result = v[i]
		}
	}
	return result
}

func BaseMax_avx512(v []float32) float32 {
	if len(v) == 0 {
		panic("vec: Max called on empty slice")
	}
	lanes := 16
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i] > result {
				result = v[i]
			}
		}
		return result
	}
	maxVec := archsimd.LoadFloat32x16Slice(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadFloat32x16Slice(v[i:])
		maxVec = maxVec.Max(va)
		va1 := archsimd.LoadFloat32x16Slice(v[i+16:])
		maxVec = maxVec.Max(va1)
		va2 := archsimd.LoadFloat32x16Slice(v[i+32:])
		maxVec = maxVec.Max(va2)
	}
	result := hwy.ReduceMax_AVX512_F32x16(maxVec)
	for ; i < len(v); i++ {
		if v[i] > result {
			result = v[i]
		}
	}
	return result
}

func BaseMax_avx512_Float64(v []float64) float64 {
	if len(v) == 0 {
		panic("vec: Max called on empty slice")
	}
	lanes := 8
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i] > result {
				result = v[i]
			}
		}
		return result
	}
	maxVec := archsimd.LoadFloat64x8Slice(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadFloat64x8Slice(v[i:])
		maxVec = maxVec.Max(va)
		va1 := archsimd.LoadFloat64x8Slice(v[i+8:])
		maxVec = maxVec.Max(va1)
		va2 := archsimd.LoadFloat64x8Slice(v[i+16:])
		maxVec = maxVec.Max(va2)
	}
	result := hwy.ReduceMax_AVX512_F64x8(maxVec)
	for ; i < len(v); i++ {
		if v[i] > result {
			result = v[i]
		}
	}
	return result
}

func BaseMax_avx512_Int32(v []int32) int32 {
	if len(v) == 0 {
		panic("vec: Max called on empty slice")
	}
	lanes := 16
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i] > result {
				result = v[i]
			}
		}
		return result
	}
	maxVec := archsimd.LoadInt32x16Slice(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadInt32x16Slice(v[i:])
		maxVec = maxVec.Max(va)
		va1 := archsimd.LoadInt32x16Slice(v[i+16:])
		maxVec = maxVec.Max(va1)
		va2 := archsimd.LoadInt32x16Slice(v[i+32:])
		maxVec = maxVec.Max(va2)
	}
	result := hwy.ReduceMax_AVX512_I32x16(maxVec)
	for ; i < len(v); i++ {
		if v[i] > result {
			result = v[i]
		}
	}
	return result
}

func BaseMax_avx512_Int64(v []int64) int64 {
	if len(v) == 0 {
		panic("vec: Max called on empty slice")
	}
	lanes := 8
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i] > result {
				result = v[i]
			}
		}
		return result
	}
	maxVec := archsimd.LoadInt64x8Slice(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadInt64x8Slice(v[i:])
		maxVec = maxVec.Max(va)
		va1 := archsimd.LoadInt64x8Slice(v[i+8:])
		maxVec = maxVec.Max(va1)
		va2 := archsimd.LoadInt64x8Slice(v[i+16:])
		maxVec = maxVec.Max(va2)
	}
	result := hwy.ReduceMax_AVX512_I64x8(maxVec)
	for ; i < len(v); i++ {
		if v[i] > result {
			result = v[i]
		}
	}
	return result
}

func BaseMax_avx512_Uint32(v []uint32) uint32 {
	if len(v) == 0 {
		panic("vec: Max called on empty slice")
	}
	lanes := 16
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i] > result {
				result = v[i]
			}
		}
		return result
	}
	maxVec := archsimd.LoadUint32x16Slice(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadUint32x16Slice(v[i:])
		maxVec = maxVec.Max(va)
		va1 := archsimd.LoadUint32x16Slice(v[i+16:])
		maxVec = maxVec.Max(va1)
		va2 := archsimd.LoadUint32x16Slice(v[i+32:])
		maxVec = maxVec.Max(va2)
	}
	result := hwy.ReduceMax_AVX512_Uint32x16(maxVec)
	for ; i < len(v); i++ {
		if v[i] > result {
			result = v[i]
		}
	}
	return result
}

func BaseMax_avx512_Uint64(v []uint64) uint64 {
	if len(v) == 0 {
		panic("vec: Max called on empty slice")
	}
	lanes := 8
	if len(v) < lanes {
		result := v[0]
		for i := 1; i < len(v); i++ {
			if v[i] > result {
				result = v[i]
			}
		}
		return result
	}
	maxVec := archsimd.LoadUint64x8Slice(v)
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadUint64x8Slice(v[i:])
		maxVec = maxVec.Max(va)
		va1 := archsimd.LoadUint64x8Slice(v[i+8:])
		maxVec = maxVec.Max(va1)
		va2 := archsimd.LoadUint64x8Slice(v[i+16:])
		maxVec = maxVec.Max(va2)
	}
	result := hwy.ReduceMax_AVX512_Uint64x8(maxVec)
	for ; i < len(v); i++ {
		if v[i] > result {
			result = v[i]
		}
	}
	return result
}

func BaseMinMax_avx512_Float16(v []hwy.Float16) (min hwy.Float16, max hwy.Float16) {
	if len(v) == 0 {
		panic("vec: MinMax called on empty slice")
	}
	lanes := 32
	if len(v) < lanes {
		min = v[0]
		max = v[0]
		for i := 1; i < len(v); i++ {
			if v[i].Float32() < min.Float32() {
				min = v[i]
			}
			if v[i].Float32() > max.Float32() {
				max = v[i]
			}
		}
		return min, max
	}
	minVec := hwy.Load(v)
	maxVec := minVec
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := hwy.Load(v[i:])
		minVec = hwy.MinF16(minVec, va)
		maxVec = hwy.MaxF16(maxVec, va)
		va1 := hwy.Load(v[i+32:])
		minVec = hwy.MinF16(minVec, va1)
		maxVec = hwy.MaxF16(maxVec, va1)
		va2 := hwy.Load(v[i+64:])
		minVec = hwy.MinF16(minVec, va2)
		maxVec = hwy.MaxF16(maxVec, va2)
	}
	min = hwy.ReduceMinF16(minVec)
	max = hwy.ReduceMaxF16(maxVec)
	for ; i < len(v); i++ {
		if v[i].Float32() < min.Float32() {
			min = v[i]
		}
		if v[i].Float32() > max.Float32() {
			max = v[i]
		}
	}
	return min, max
}

func BaseMinMax_avx512_BFloat16(v []hwy.BFloat16) (min hwy.BFloat16, max hwy.BFloat16) {
	if len(v) == 0 {
		panic("vec: MinMax called on empty slice")
	}
	lanes := 32
	if len(v) < lanes {
		min = v[0]
		max = v[0]
		for i := 1; i < len(v); i++ {
			if v[i].Float32() < min.Float32() {
				min = v[i]
			}
			if v[i].Float32() > max.Float32() {
				max = v[i]
			}
		}
		return min, max
	}
	minVec := hwy.Load(v)
	maxVec := minVec
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := hwy.Load(v[i:])
		minVec = hwy.MinBF16(minVec, va)
		maxVec = hwy.MaxBF16(maxVec, va)
		va1 := hwy.Load(v[i+32:])
		minVec = hwy.MinBF16(minVec, va1)
		maxVec = hwy.MaxBF16(maxVec, va1)
		va2 := hwy.Load(v[i+64:])
		minVec = hwy.MinBF16(minVec, va2)
		maxVec = hwy.MaxBF16(maxVec, va2)
	}
	min = hwy.ReduceMinBF16(minVec)
	max = hwy.ReduceMaxBF16(maxVec)
	for ; i < len(v); i++ {
		if v[i].Float32() < min.Float32() {
			min = v[i]
		}
		if v[i].Float32() > max.Float32() {
			max = v[i]
		}
	}
	return min, max
}

func BaseMinMax_avx512(v []float32) (min float32, max float32) {
	if len(v) == 0 {
		panic("vec: MinMax called on empty slice")
	}
	lanes := 16
	if len(v) < lanes {
		min = v[0]
		max = v[0]
		for i := 1; i < len(v); i++ {
			if v[i] < min {
				min = v[i]
			}
			if v[i] > max {
				max = v[i]
			}
		}
		return min, max
	}
	minVec := archsimd.LoadFloat32x16Slice(v)
	maxVec := minVec
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadFloat32x16Slice(v[i:])
		minVec = minVec.Min(va)
		maxVec = maxVec.Max(va)
		va1 := archsimd.LoadFloat32x16Slice(v[i+16:])
		minVec = minVec.Min(va1)
		maxVec = maxVec.Max(va1)
		va2 := archsimd.LoadFloat32x16Slice(v[i+32:])
		minVec = minVec.Min(va2)
		maxVec = maxVec.Max(va2)
	}
	min = hwy.ReduceMin_AVX512_F32x16(minVec)
	max = hwy.ReduceMax_AVX512_F32x16(maxVec)
	for ; i < len(v); i++ {
		if v[i] < min {
			min = v[i]
		}
		if v[i] > max {
			max = v[i]
		}
	}
	return min, max
}

func BaseMinMax_avx512_Float64(v []float64) (min float64, max float64) {
	if len(v) == 0 {
		panic("vec: MinMax called on empty slice")
	}
	lanes := 8
	if len(v) < lanes {
		min = v[0]
		max = v[0]
		for i := 1; i < len(v); i++ {
			if v[i] < min {
				min = v[i]
			}
			if v[i] > max {
				max = v[i]
			}
		}
		return min, max
	}
	minVec := archsimd.LoadFloat64x8Slice(v)
	maxVec := minVec
	var i int
	for i = lanes; i+lanes*3 <= len(v); i += lanes * 3 {
		va := archsimd.LoadFloat64x8Slice(v[i:])
		minVec = minVec.Min(va)
		maxVec = maxVec.Max(va)
		va1 := archsimd.LoadFloat64x8Slice(v[i+8:])
		minVec = minVec.Min(va1)
		maxVec = maxVec.Max(va1)
		va2 := archsimd.LoadFloat64x8Slice(v[i+16:])
		minVec = minVec.Min(va2)
		maxVec = maxVec.Max(va2)
	}
	min = hwy.ReduceMin_AVX512_F64x8(minVec)
	max = hwy.ReduceMax_AVX512_F64x8(maxVec)
	for ; i < len(v); i++ {
		if v[i] < min {
			min = v[i]
		}
		if v[i] > max {
			max = v[i]
		}
	}
	return min, max
}
