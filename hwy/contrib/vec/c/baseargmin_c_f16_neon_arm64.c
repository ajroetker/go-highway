...
// BaseArgmin for NEON hwy.Float16
...
#ifndef GOAT_PARSER
static long scalarArgmin(float16_t *v, long len_v) {
    long bestIdx = 0;
    float16_t minVal;
    long foundValid = 0;
...
void argmin_c_f16_neon(float16_t *v, long *plen_v, long *pout_result) {
    long len_v = *plen_v;
    long lanes = 8;
    if (len_v < lanes) {
    ...
    float16x8_t minVals = vld1q_f16((float16_t*)(v));
    float16x8_t minIdxs = hwy_iota_f16();
    long i = lanes;
    #pragma clang loop vectorize(disable) interleave(disable)
    for (; i + lanes <= len_v; i += lanes) {
        ...
        uint16x8_t mask = vcltq_f16(vals, minVals);
        minVals = vbslq_f16(mask, vals, minVals);
        minIdxs = vbslq_f16(mask, curIdxs, minIdxs);
    }
    float16_t valsData[8];
    vst1q_f16((float16_t*)(valsData), minVals);
    float16_t idxsData[8];
    vst1q_f16((float16_t*)(idxsData), minIdxs);
    long bestIdx = 0;
    float16_t minVal;
    long foundValid = 0;
    #pragma clang loop vectorize(disable) interleave(disable)
