...
// BaseArgmax for NEON hwy.Float16
...
#ifndef GOAT_PARSER
static long scalarArgmax(float16_t *v, long len_v) {
    long bestIdx = 0;
    float16_t maxVal;
    long foundValid = 0;
...
void argmax_c_f16_neon(float16_t *v, long *plen_v, long *pout_result) {
    long len_v = *plen_v;
    long lanes = 8;
    if (len_v < lanes) {
        *pout_result = scalarArgmax(v, len_v);
        return;
    }
    float16x8_t maxVals = vld1q_f16((float16_t*)(v));
    float16x8_t maxIdxs = hwy_iota_f16();
    long i = lanes;
    #pragma clang loop vectorize(disable) interleave(disable)
    for (; i + lanes <= len_v; i += lanes) {
        float16x8_t vals = vld1q_f16((float16_t*)(v + i));
        float16x8_t curIdxs = vaddq_f16(vdupq_n_f16((unsigned short)(i)), hwy_iota_f16());
        uint16x8_t mask = vcgtq_f16(vals, maxVals);
        maxVals = vbslq_f16(mask, vals, maxVals);
        maxIdxs = vbslq_f16(mask, curIdxs, maxIdxs);
    }
    float16_t valsData[8];
    vst1q_f16((float16_t*)(valsData), maxVals);
    float16_t idxsData[8];
    vst1q_f16((float16_t*)(idxsData), maxIdxs);
    long bestIdx = 0;
    float16_t maxVal;
    long foundValid = 0;
    #pragma clang loop vectorize(disable) interleave(disable)
    for (long j = 0; j < lanes; j++) {
        float16_t val = valsData[j];
        if (val != val) {
            continue;
        }
