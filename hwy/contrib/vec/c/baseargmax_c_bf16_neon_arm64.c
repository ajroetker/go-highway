...
// BaseArgmax for NEON hwy.BFloat16
...
#ifndef GOAT_PARSER
static long scalarArgmax(unsigned short *v, long len_v) {
    long bestIdx = 0;
    float maxVal = 0;
    long foundValid = 0;
...
void argmax_c_bf16_neon(unsigned short *v, long *plen_v, long *pout_result) {
    long len_v = *plen_v;
    long lanes = 8;
    if (len_v < lanes) {
    ...
    bfloat16x8_t maxVals = vld1q_bf16((bfloat16_t*)(v));
    bfloat16x8_t maxIdxs = hwy_iota_bf16();
    long i = lanes;
    #pragma clang loop vectorize(disable) interleave(disable)
    for (; i + lanes <= len_v; i += lanes) {
        ...
        uint16x8_t mask = bf16_gt_q(vals, maxVals);
        maxVals = bf16_ifelse_q(mask, vals, maxVals);
        maxIdxs = bf16_ifelse_q(mask, curIdxs, maxIdxs);
    }
    float valsData[8];
    vst1q_bf16((bfloat16_t*)(valsData), maxVals);
    float idxsData[8];
    vst1q_bf16((bfloat16_t*)(idxsData), maxIdxs);
    long bestIdx = 0;
    float maxVal = 0;
    long foundValid = 0;
    #pragma clang loop vectorize(disable) interleave(disable)
