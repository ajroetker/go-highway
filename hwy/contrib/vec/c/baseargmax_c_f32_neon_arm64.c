...
#ifndef GOAT_PARSER
static long scalarArgmax(float *v, long len_v) {
    long bestIdx = 0;
    float maxVal = 0;
    long foundValid = 0;
...
void argmax_c_f32_neon(float *v, long *plen_v, long *pout_result) {
    long len_v = *plen_v;
    long lanes = 4;
    if (len_v < lanes) {
    ...
    float32x4_t maxVals = vld1q_f32(v);
    float32x4_t maxIdxs = hwy_iota_f32();
    long i = lanes;
    #pragma clang loop vectorize(disable) interleave(disable)
    for (; i + lanes <= len_v; i += lanes) {
        float32x4_t vals = vld1q_f32(v + i);
        float32x4_t curIdxs = vaddq_f32(vdupq_n_f32((float)(i)), hwy_iota_f32());
        uint32x4_t mask = vcgtq_f32(vals, maxVals);
        maxVals = vbslq_f32(mask, vals, maxVals);
        maxIdxs = vbslq_f32(mask, curIdxs, maxIdxs);
    }
    float valsData[4];
    vst1q_f32(valsData, maxVals);
    float idxsData[4];
    vst1q_f32(idxsData, maxIdxs);
    long bestIdx = 0;
    float maxVal = 0;
    long foundValid = 0;
    #pragma clang loop vectorize(disable) interleave(disable)
