...
#ifndef GOAT_PARSER
static long scalarArgmax(double *v, long len_v) {
    long bestIdx = 0;
    double maxVal = 0;
    long foundValid = 0;
...
void argmax_c_f64_neon(double *v, long *plen_v, long *pout_result) {
    long len_v = *plen_v;
    long lanes = 2;
    if (len_v < lanes) {
    ...
    float64x2_t maxVals = vld1q_f64(v);
    float64x2_t maxIdxs = hwy_iota_f64();
    long i = lanes;
    #pragma clang loop vectorize(disable) interleave(disable)
    for (; i + lanes <= len_v; i += lanes) {
        float64x2_t vals = vld1q_f64(v + i);
        float64x2_t curIdxs = vaddq_f64(vdupq_n_f64((double)(i)), hwy_iota_f64());
        uint64x2_t mask = vcgtq_f64(vals, maxVals);
        maxVals = vbslq_f64(mask, vals, maxVals);
        maxIdxs = vbslq_f64(mask, curIdxs, maxIdxs);
    }
    double valsData[2];
    vst1q_f64(valsData, maxVals);
    double idxsData[2];
    vst1q_f64(idxsData, maxIdxs);
    long bestIdx = 0;
    double maxVal = 0;
    long foundValid = 0;
    #pragma clang loop vectorize(disable) interleave(disable)
