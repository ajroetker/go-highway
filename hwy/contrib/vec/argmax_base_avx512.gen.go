...
func BaseArgmax_avx512_Float16(v []hwy.Float16) int {
	...
	maxVals := hwy.Load(v)
	maxIdxs := archsimd.Iota()
	i := lanes
	for ; i+lanes <= len(v); i += lanes {
		vals := hwy.Load(v[i:])
		curIdxs := hwy.AddF16(hwy.Set(hwy.Float16(i)), archsimd.Iota())
...
func BaseArgmax_avx512_BFloat16(v []hwy.BFloat16) int {
	...
	for ; i+lanes <= len(v); i += lanes {
		vals := hwy.Load(v[i:])
		curIdxs := hwy.AddBF16(hwy.Set(hwy.BFloat16(i)), archsimd.Iota())
...
func BaseArgmax_avx512(v []float32) int {
	...
	for ; i+lanes <= len(v); i += lanes {
		...
		curIdxs := archsimd.BroadcastFloat32x16(float32(i)).Add(archsimd.Iota())
...
func BaseArgmax_avx512_Float64(v []float64) int {
	...
	for ; i+lanes <= len(v); i += lanes {
		...
		curIdxs := archsimd.BroadcastFloat64x8(float64(i)).Add(archsimd.Iota())
		mask := vals.Greater(maxVals)
