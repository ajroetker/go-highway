...
func BaseBatchDot_avx512_Float16(query []hwy.Float16, data []hwy.Float16, dots []hwy.Float16, count int, dims int) {
	...
	for i := range count {
		...
		for j = 0; j+lanes <= dims; j += lanes {
			vq := asm.LoadFloat16x16AVX512Ptr(unsafe.Pointer(&query[j]))
			vd := asm.LoadFloat16x16AVX512Ptr(unsafe.Pointer(&dataVec[j]))
			prod := vq.Mul(vd)
			sum = sum.Add(prod)
		}
...
func BaseBatchDot_avx512_BFloat16(query []hwy.BFloat16, data []hwy.BFloat16, dots []hwy.BFloat16, count int, dims int) {
	...
	for i := range count {
		...
		var j int
		for j = 0; j+lanes <= dims; j += lanes {
			vq := asm.LoadBFloat16x16AVX512Ptr(unsafe.Pointer(&query[j]))
			vd := asm.LoadBFloat16x16AVX512Ptr(unsafe.Pointer(&dataVec[j]))
			prod := vq.Mul(vd)
			sum = sum.Add(prod)
		}
		result := sum.ReduceSum()
