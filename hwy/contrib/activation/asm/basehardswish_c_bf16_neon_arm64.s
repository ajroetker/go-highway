//go:build !noasm && arm64
// Code generated by GoAT. DO NOT EDIT.
// versions:
// 	clang   22.1.0
// 	objdump 2.46.0.20260210
// flags: -march=armv8.6-a+bf16+simd -fno-builtin-memset -O3
// source: /Users/ajroetker/go/src/github.com/ajroetker/go-highway/hwy/contrib/activation/asm/basehardswish_c_bf16_neon_arm64.c

TEXT ·hardswish_c_bf16_neon(SB), $0-24
	MOVD input+0(FP), R0
	MOVD output+8(FP), R1
	MOVD plen_input+16(FP), R2
	WORD $0xf9400048           // ldr	x8, [x2]
	CBZ  R8, BB0_8
	WORD $0xf100211f           // cmp	x8, #8
	BGE  BB0_3
	WORD $0xd2800009           // mov	x9, #0                          ; =0x0
	B    BB0_5

BB0_3:
	WORD $0xd2800009 // mov	x9, #0                          ; =0x0
	WORD $0x6f00e400 // movi.2d	v0, #0000000000000000
	WORD $0x4f000421 // movi.4s	v1, #1
	WORD $0x4f03c7e2 // movi.4s	v2, #127, msl #8
	WORD $0x6f06e583 // movi.2d	v3, #0xffff0000ffff0000
	WORD $0xaa0103ea // mov	x10, x1
	WORD $0xaa0003eb // mov	x11, x0
	WORD $0x4f004424 // movi.4s	v4, #1, lsl #16

BB0_4:
	WORD $0x3cc10565 // ldr	q5, [x11], #16
	WORD $0x2e6138a6 // shll.4s	v6, v5, #16
	WORD $0x6e6138a5 // shll2.4s	v5, v5, #16
	WORD $0x6e20dcc7 // fmul.4s	v7, v6, v0
	WORD $0x6f3004f0 // ushr.4s	v16, v7, #16
	WORD $0x4ea284e7 // add.4s	v7, v7, v2
	WORD $0x4e211e10 // and.16b	v16, v16, v1
	WORD $0x4eb084e7 // add.4s	v7, v7, v16
	WORD $0x6e20dcb0 // fmul.4s	v16, v5, v0
	WORD $0x6f300611 // ushr.4s	v17, v16, #16
	WORD $0x4e211e31 // and.16b	v17, v17, v1
	WORD $0x4ea28610 // add.4s	v16, v16, v2
	WORD $0x4e231ce7 // and.16b	v7, v7, v3
	WORD $0x4eb18610 // add.4s	v16, v16, v17
	WORD $0x4e231e10 // and.16b	v16, v16, v3
	WORD $0x4e20d4e7 // fadd.4s	v7, v7, v0
	WORD $0x6f3004f1 // ushr.4s	v17, v7, #16
	WORD $0x4ea284e7 // add.4s	v7, v7, v2
	WORD $0x4e211e31 // and.16b	v17, v17, v1
	WORD $0x4eb184e7 // add.4s	v7, v7, v17
	WORD $0x4e20d610 // fadd.4s	v16, v16, v0
	WORD $0x6f300611 // ushr.4s	v17, v16, #16
	WORD $0x4e211e31 // and.16b	v17, v17, v1
	WORD $0x4ea28610 // add.4s	v16, v16, v2
	WORD $0x4e231ce7 // and.16b	v7, v7, v3
	WORD $0x4eb18610 // add.4s	v16, v16, v17
	WORD $0x4e231e10 // and.16b	v16, v16, v3
	WORD $0x4e20f4e7 // fmax.4s	v7, v7, v0
	WORD $0x6f3004f1 // ushr.4s	v17, v7, #16
	WORD $0x4ea284e7 // add.4s	v7, v7, v2
	WORD $0x4e211e31 // and.16b	v17, v17, v1
	WORD $0x4eb184e7 // add.4s	v7, v7, v17
	WORD $0x4e20f610 // fmax.4s	v16, v16, v0
	WORD $0x6f300611 // ushr.4s	v17, v16, #16
	WORD $0x4e211e31 // and.16b	v17, v17, v1
	WORD $0x4ea28610 // add.4s	v16, v16, v2
	WORD $0x4e231ce7 // and.16b	v7, v7, v3
	WORD $0x4eb18610 // add.4s	v16, v16, v17
	WORD $0x4e231e10 // and.16b	v16, v16, v3
	WORD $0x4ea4f4e7 // fmin.4s	v7, v7, v4
	WORD $0x6f3004f1 // ushr.4s	v17, v7, #16
	WORD $0x4ea284e7 // add.4s	v7, v7, v2
	WORD $0x4e211e31 // and.16b	v17, v17, v1
	WORD $0x4eb184e7 // add.4s	v7, v7, v17
	WORD $0x4ea4f610 // fmin.4s	v16, v16, v4
	WORD $0x6f300611 // ushr.4s	v17, v16, #16
	WORD $0x4e211e31 // and.16b	v17, v17, v1
	WORD $0x4ea28610 // add.4s	v16, v16, v2
	WORD $0x4e231ce7 // and.16b	v7, v7, v3
	WORD $0x4eb18610 // add.4s	v16, v16, v17
	WORD $0x4e231e10 // and.16b	v16, v16, v3
	WORD $0x6e27dcc6 // fmul.4s	v6, v6, v7
	WORD $0x6e30dca5 // fmul.4s	v5, v5, v16
	WORD $0x6f3004a7 // ushr.4s	v7, v5, #16
	WORD $0x4e211ce7 // and.16b	v7, v7, v1
	WORD $0x4ea784a5 // add.4s	v5, v5, v7
	WORD $0x6f3004c7 // ushr.4s	v7, v6, #16
	WORD $0x4e211ce7 // and.16b	v7, v7, v1
	WORD $0x4ea784c6 // add.4s	v6, v6, v7
	WORD $0x0e6240c6 // addhn.4h	v6, v6, v2
	WORD $0x4e6240a6 // addhn2.8h	v6, v5, v2
	WORD $0x3c810546 // str	q6, [x10], #16
	WORD $0x9100412c // add	x12, x9, #16
	WORD $0x91002129 // add	x9, x9, #8
	WORD $0xeb08019f // cmp	x12, x8
	BLE  BB0_4

BB0_5:
	WORD $0xeb090108 // subs	x8, x8, x9
	BLE  BB0_8
	WORD $0xd37ff92a // lsl	x10, x9, #1
	WORD $0x8b0a0029 // add	x9, x1, x10
	WORD $0x8b0a000a // add	x10, x0, x10
	WORD $0x1e631000 // fmov	d0, #6.00000000
	WORD $0x1e6c1001 // fmov	d1, #0.50000000
	WORD $0x1e6e1002 // fmov	d2, #1.00000000
	WORD $0x528fffeb // mov	w11, #32767                     ; =0x7fff

BB0_7:
	WORD $0x7840254c // ldrh	w12, [x10], #2
	WORD $0x1e630183 // ucvtf	d3, w12
	WORD $0x1e601864 // fdiv	d4, d3, d0
	WORD $0x1e612884 // fadd	d4, d4, d1
	WORD $0x1e622080 // fcmp	d4, d2
	WORD $0x1e64cc44 // fcsel	d4, d2, d4, gt
	WORD $0x1e630883 // fmul	d3, d4, d3
	WORD $0x1e78006c // fcvtzs	w12, d3
	WORD $0x1e230183 // ucvtf	s3, w12
	WORD $0x1e26006c // fmov	w12, s3
	WORD $0x5310418d // ubfx	w13, w12, #16, #1
	WORD $0x0b0d018c // add	w12, w12, w13
	WORD $0x0b0b018c // add	w12, w12, w11
	WORD $0x53107d8c // lsr	w12, w12, #16
	WORD $0x7800252c // strh	w12, [x9], #2
	WORD $0xf1000508 // subs	x8, x8, #1
	BNE  BB0_7

BB0_8:
	RET
