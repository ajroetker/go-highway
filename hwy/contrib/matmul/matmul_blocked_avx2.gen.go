...
func BaseBlockedMatMul_avx2_Float16(a []hwy.Float16, b []hwy.Float16, c []hwy.Float16, m int, n int, k int) {
	...
	for ; idx+lanes*2 <= total; idx += lanes * 2 {
		...
		vZero.StorePtr(unsafe.Pointer(&c[idx+8]))
	}
	if idx < total {
				...
				for ; j+8 <= jEnd; j += lanes {
					...
					if remaining >= lanes {
						...
						for p := 0; p < k; p++ {
							...
							vB := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[p*n+j]))
				...
				for ; j+8 <= jEnd; j += lanes {
					...
					if remaining >= lanes {
						...
						for p := 0; p < k; p++ {
							...
							vB := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[p*n+j]))
							acc0 = vA0.MulAdd(vB, acc0)
							acc1 = vA1.MulAdd(vB, acc1)
							acc2 = vA2.MulAdd(vB, acc2)
							acc3 = vA3.MulAdd(vB, acc3)
...
func BaseBlockedMatMul_avx2(a []float32, b []float32, c []float32, m int, n int, k int) {
	...
	for ; idx+lanes*2 <= total; idx += lanes * 2 {
		...
		vZero.Store((*[8]float32)(unsafe.Pointer(&c[idx+8])))
	}
	if idx < total {
...
func BaseBlockedMatMul_avx2_Float64(a []float64, b []float64, c []float64, m int, n int, k int) {
	...
	for ; idx+lanes*2 <= total; idx += lanes * 2 {
		...
		vZero.Store((*[4]float64)(unsafe.Pointer(&c[idx+4])))
	}
	if idx < total {
