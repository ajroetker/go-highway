	...
	if m == 1 {
		...
		for j = 0; j+3 < n; j += 4 {
			...
			for p = 0; p+lanes <= k; p += lanes {
				...
				acc0 = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow0+p])), acc0)
				acc1 = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow1+p])), acc1)
				acc2 = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow2+p])), acc2)
				acc3 = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow3+p])), acc3)
			}
			s0 := acc0.ReduceSum()
			s1 := acc1.ReduceSum()
			s2 := acc2.ReduceSum()
		...
		for ; j < n; j++ {
			...
			var p int
			for p = 0; p+lanes <= k; p += lanes {
				acc = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow+p])).MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow+p])), acc)
			}
			sum := acc.ReduceSum()
			for ; p < k; p++ {
	...
	for i = 0; i+3 < m; i += 4 {
		...
		for j := range n {
			...
			for p = 0; p+lanes <= k; p += lanes {
				...
				acc0 = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow0+p])).MulAdd(vB, acc0)
				acc1 = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow1+p])).MulAdd(vB, acc1)
				acc2 = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow2+p])).MulAdd(vB, acc2)
				acc3 = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow3+p])).MulAdd(vB, acc3)
			}
			s0 := acc0.ReduceSum()
			s1 := acc1.ReduceSum()
			s2 := acc2.ReduceSum()
	...
	if m == 1 {
		...
		for j = 0; j+3 < n; j += 4 {
			...
			for p = 0; p+lanes <= k; p += lanes {
				...
				acc0 = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow0+p])), acc0)
				acc1 = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow1+p])), acc1)
				acc2 = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow2+p])), acc2)
				acc3 = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow3+p])), acc3)
			}
			s0 := acc0.ReduceSum()
			s1 := acc1.ReduceSum()
			s2 := acc2.ReduceSum()
		...
		for ; j < n; j++ {
			...
			var p int
			for p = 0; p+lanes <= k; p += lanes {
				acc = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow+p])).MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRow+p])), acc)
			}
			sum := acc.ReduceSum()
			for ; p < k; p++ {
				sum += a[aRow+p].Float32() * b[bRow+p].Float32()
	...
	for i = 0; i+3 < m; i += 4 {
		...
		for j := range n {
			...
			for p = 0; p+lanes <= k; p += lanes {
				...
				acc0 = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow0+p])).MulAdd(vB, acc0)
				acc1 = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow1+p])).MulAdd(vB, acc1)
				acc2 = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow2+p])).MulAdd(vB, acc2)
				acc3 = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&a[aRow3+p])).MulAdd(vB, acc3)
			}
			s0 := acc0.ReduceSum()
			s1 := acc1.ReduceSum()
			s2 := acc2.ReduceSum()
...
func BaseMatMulKLast_avx2(a []float32, b []float32, c []float32, m int, n int, k int) {
	...
	if m == 1 {
		...
		for j = 0; j+3 < n; j += 4 {
			...
			var p int
			for p = 0; p+lanes <= k; p += lanes {
				vA := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&a[aRow+p])))
				acc0 = vA.MulAdd(archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&b[bRow0+p]))), acc0)
				acc1 = vA.MulAdd(archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&b[bRow1+p]))), acc1)
				acc2 = vA.MulAdd(archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&b[bRow2+p]))), acc2)
				acc3 = vA.MulAdd(archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&b[bRow3+p]))), acc3)
			}
		...
		for ; j < n; j++ {
			...
			var p int
			for p = 0; p+lanes <= k; p += lanes {
				acc = archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&a[aRow+p]))).MulAdd(archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&b[bRow+p]))), acc)
			}
	...
	for i = 0; i+3 < m; i += 4 {
		...
		for j := range n {
			...
			var p int
			for p = 0; p+lanes <= k; p += lanes {
				vB := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&b[bRow+p])))
				acc0 = archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&a[aRow0+p]))).MulAdd(vB, acc0)
				acc1 = archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&a[aRow1+p]))).MulAdd(vB, acc1)
				acc2 = archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&a[aRow2+p]))).MulAdd(vB, acc2)
				acc3 = archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&a[aRow3+p]))).MulAdd(vB, acc3)
			}
			s0 := hwy.ReduceSum_AVX2_F32x8(acc0)
			s1 := hwy.ReduceSum_AVX2_F32x8(acc1)
			s2 := hwy.ReduceSum_AVX2_F32x8(acc2)
...
func BaseMatMulKLast_avx2_Float64(a []float64, b []float64, c []float64, m int, n int, k int) {
	...
	if m == 1 {
		...
		for j = 0; j+3 < n; j += 4 {
			...
			var p int
			for p = 0; p+lanes <= k; p += lanes {
				vA := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&a[aRow+p])))
				acc0 = vA.MulAdd(archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&b[bRow0+p]))), acc0)
				acc1 = vA.MulAdd(archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&b[bRow1+p]))), acc1)
				acc2 = vA.MulAdd(archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&b[bRow2+p]))), acc2)
				acc3 = vA.MulAdd(archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&b[bRow3+p]))), acc3)
			}
		...
		for ; j < n; j++ {
			...
			var p int
			for p = 0; p+lanes <= k; p += lanes {
				acc = archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&a[aRow+p]))).MulAdd(archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&b[bRow+p]))), acc)
			}
	...
	for i = 0; i+3 < m; i += 4 {
		...
		for j := range n {
			...
			var p int
			for p = 0; p+lanes <= k; p += lanes {
				vB := archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&b[bRow+p])))
				acc0 = archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&a[aRow0+p]))).MulAdd(vB, acc0)
				acc1 = archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&a[aRow1+p]))).MulAdd(vB, acc1)
				acc2 = archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&a[aRow2+p]))).MulAdd(vB, acc2)
				acc3 = archsimd.LoadFloat64x4Array((*[4]float64)(unsafe.Pointer(&a[aRow3+p]))).MulAdd(vB, acc3)
			}
			s0 := hwy.ReduceSum_AVX2_F64x4(acc0)
			s1 := hwy.ReduceSum_AVX2_F64x4(acc1)
			s2 := hwy.ReduceSum_AVX2_F64x4(acc2)
