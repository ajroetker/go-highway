	...
	for i := range blockDim {
		...
		for j = 0; j+tileJ <= blockDim; j += tileJ {
			...
			for k := range blockDim {
				...
				acc0 = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j:][0])), acc0)
				acc1 = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+lanes:][0])), acc1)
				acc2 = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+2*lanes:][0])), acc2)
				acc3 = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+3*lanes:][0])), acc3)
			}
			vC := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j:][0]))
			vC.Add(acc0).StorePtr(unsafe.Pointer(&c[cRowStart+j:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j+lanes:][0]))
			vC.Add(acc1).StorePtr(unsafe.Pointer(&c[cRowStart+j+lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j+2*lanes:][0]))
			vC.Add(acc2).StorePtr(unsafe.Pointer(&c[cRowStart+j+2*lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j+3*lanes:][0]))
		...
		for ; j+lanes <= blockDim; j += lanes {
			...
			for k := range blockDim {
				aik := aT[k*blockDim+i]
				vA := asm.BroadcastFloat16x8AVX2(uint16(aik))
				acc = vA.MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[k*blockDim+j:][0])), acc)
			}
			vC := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j:][0]))
	...
	for i := range blockDim {
		...
		for j = 0; j+tileJ <= blockDim; j += tileJ {
			...
			for k := range blockDim {
				...
				bRowStart := k * blockDim
				acc0 = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j:][0])), acc0)
				acc1 = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+lanes:][0])), acc1)
				acc2 = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+2*lanes:][0])), acc2)
				acc3 = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+3*lanes:][0])), acc3)
			}
			vC := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j:][0]))
			vC.Add(acc0).StorePtr(unsafe.Pointer(&c[cRowStart+j:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j+lanes:][0]))
			vC.Add(acc1).StorePtr(unsafe.Pointer(&c[cRowStart+j+lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j+2*lanes:][0]))
			vC.Add(acc2).StorePtr(unsafe.Pointer(&c[cRowStart+j+2*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j+3*lanes:][0]))
			vC.Add(acc3).StorePtr(unsafe.Pointer(&c[cRowStart+j+3*lanes:][0]))
		}
		for ; j+lanes <= blockDim; j += lanes {
			...
			for k := range blockDim {
				aik := aT[k*blockDim+i]
				vA := asm.BroadcastBFloat16x8AVX2(uint16(aik))
				acc = vA.MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[k*blockDim+j:][0])), acc)
			}
			vC := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j:][0]))
			vC.Add(acc).StorePtr(unsafe.Pointer(&c[cRowStart+j:][0]))
		}
...
func BaseBlockMulAdd2_avx2_Float16(aT []hwy.Float16, b []hwy.Float16, c []hwy.Float16, blockDim int) {
	...
	for i = 0; i+1 < blockDim; i += 2 {
		...
		for j = 0; j+tileJ <= blockDim; j += tileJ {
			...
			for k := range blockDim {
				...
				vB2 := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+2*lanes:][0]))
				vB3 := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+3*lanes:][0]))
			...
			vC := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0Start+j:][0]))
			vC.Add(acc00).StorePtr(unsafe.Pointer(&c[cRow0Start+j:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0Start+j+lanes:][0]))
			vC.Add(acc01).StorePtr(unsafe.Pointer(&c[cRow0Start+j+lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0Start+j+2*lanes:][0]))
			vC.Add(acc02).StorePtr(unsafe.Pointer(&c[cRow0Start+j+2*lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0Start+j+3*lanes:][0]))
			vC.Add(acc03).StorePtr(unsafe.Pointer(&c[cRow0Start+j+3*lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1Start+j:][0]))
			vC.Add(acc10).StorePtr(unsafe.Pointer(&c[cRow1Start+j:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1Start+j+lanes:][0]))
			vC.Add(acc11).StorePtr(unsafe.Pointer(&c[cRow1Start+j+lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1Start+j+2*lanes:][0]))
			vC.Add(acc12).StorePtr(unsafe.Pointer(&c[cRow1Start+j+2*lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1Start+j+3*lanes:][0]))
	...
	if i < blockDim {
		...
		for ; j+lanes <= blockDim; j += lanes {
			acc := asm.ZeroFloat16x8AVX2()
			for k := range blockDim {
				acc = asm.BroadcastFloat16x8AVX2(uint16(aT[k*blockDim+i])).MulAdd(asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&b[k*blockDim+j:][0])), acc)
			}
			vC := asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j:][0]))
			vC.Add(acc).StorePtr(unsafe.Pointer(&c[cRowStart+j:][0]))
		}
...
func BaseBlockMulAdd2_avx2_BFloat16(aT []hwy.BFloat16, b []hwy.BFloat16, c []hwy.BFloat16, blockDim int) {
	...
	for i = 0; i+1 < blockDim; i += 2 {
		...
		for j = 0; j+tileJ <= blockDim; j += tileJ {
			...
			for k := range blockDim {
				...
				vB2 := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+2*lanes:][0]))
				vB3 := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[bRowStart+j+3*lanes:][0]))
				acc00 = vA0.MulAdd(vB0, acc00)
				acc01 = vA0.MulAdd(vB1, acc01)
				acc02 = vA0.MulAdd(vB2, acc02)
				acc03 = vA0.MulAdd(vB3, acc03)
			...
			vC := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0Start+j:][0]))
			vC.Add(acc00).StorePtr(unsafe.Pointer(&c[cRow0Start+j:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0Start+j+lanes:][0]))
			vC.Add(acc01).StorePtr(unsafe.Pointer(&c[cRow0Start+j+lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0Start+j+2*lanes:][0]))
			vC.Add(acc02).StorePtr(unsafe.Pointer(&c[cRow0Start+j+2*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0Start+j+3*lanes:][0]))
			vC.Add(acc03).StorePtr(unsafe.Pointer(&c[cRow0Start+j+3*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1Start+j:][0]))
			vC.Add(acc10).StorePtr(unsafe.Pointer(&c[cRow1Start+j:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1Start+j+lanes:][0]))
			vC.Add(acc11).StorePtr(unsafe.Pointer(&c[cRow1Start+j+lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1Start+j+2*lanes:][0]))
			vC.Add(acc12).StorePtr(unsafe.Pointer(&c[cRow1Start+j+2*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1Start+j+3*lanes:][0]))
			vC.Add(acc13).StorePtr(unsafe.Pointer(&c[cRow1Start+j+3*lanes:][0]))
		}
		for ; j+lanes <= blockDim; j += lanes {
			acc0 := asm.ZeroBFloat16x8AVX2()
	...
	if i < blockDim {
		...
		for ; j+lanes <= blockDim; j += lanes {
			acc := asm.ZeroBFloat16x8AVX2()
			for k := range blockDim {
				acc = asm.BroadcastBFloat16x8AVX2(uint16(aT[k*blockDim+i])).MulAdd(asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&b[k*blockDim+j:][0])), acc)
			}
			vC := asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRowStart+j:][0]))
			vC.Add(acc).StorePtr(unsafe.Pointer(&c[cRowStart+j:][0]))
		}
...
func BaseBlockMulAdd4_avx2_Float16(aT []hwy.Float16, b []hwy.Float16, c []hwy.Float16, blockDim int) {
	...
	for i = 0; i+3 < blockDim; i += 4 {
		...
		for j = 0; j+tileJ <= blockDim; j += tileJ {
			...
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0+j+2*lanes:][0]))
			vC.Add(acc02).StorePtr(unsafe.Pointer(&c[cRow0+j+2*lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0+j+3*lanes:][0]))
			...
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1+j+2*lanes:][0]))
			vC.Add(acc12).StorePtr(unsafe.Pointer(&c[cRow1+j+2*lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1+j+3*lanes:][0]))
			...
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow2+j+2*lanes:][0]))
			vC.Add(acc22).StorePtr(unsafe.Pointer(&c[cRow2+j+2*lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow2+j+3*lanes:][0]))
			...
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow3+j+2*lanes:][0]))
			vC.Add(acc32).StorePtr(unsafe.Pointer(&c[cRow3+j+2*lanes:][0]))
			vC = asm.LoadFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow3+j+3*lanes:][0]))
...
func BaseBlockMulAdd4_avx2_BFloat16(aT []hwy.BFloat16, b []hwy.BFloat16, c []hwy.BFloat16, blockDim int) {
	...
	for i = 0; i+3 < blockDim; i += 4 {
		...
		for j = 0; j+tileJ <= blockDim; j += tileJ {
			...
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0+j+lanes:][0]))
			vC.Add(acc01).StorePtr(unsafe.Pointer(&c[cRow0+j+lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0+j+2*lanes:][0]))
			vC.Add(acc02).StorePtr(unsafe.Pointer(&c[cRow0+j+2*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow0+j+3*lanes:][0]))
			vC.Add(acc03).StorePtr(unsafe.Pointer(&c[cRow0+j+3*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1+j:][0]))
			vC.Add(acc10).StorePtr(unsafe.Pointer(&c[cRow1+j:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1+j+lanes:][0]))
			vC.Add(acc11).StorePtr(unsafe.Pointer(&c[cRow1+j+lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1+j+2*lanes:][0]))
			vC.Add(acc12).StorePtr(unsafe.Pointer(&c[cRow1+j+2*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow1+j+3*lanes:][0]))
			vC.Add(acc13).StorePtr(unsafe.Pointer(&c[cRow1+j+3*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow2+j:][0]))
			vC.Add(acc20).StorePtr(unsafe.Pointer(&c[cRow2+j:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow2+j+lanes:][0]))
			vC.Add(acc21).StorePtr(unsafe.Pointer(&c[cRow2+j+lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow2+j+2*lanes:][0]))
			vC.Add(acc22).StorePtr(unsafe.Pointer(&c[cRow2+j+2*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow2+j+3*lanes:][0]))
			vC.Add(acc23).StorePtr(unsafe.Pointer(&c[cRow2+j+3*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow3+j:][0]))
			vC.Add(acc30).StorePtr(unsafe.Pointer(&c[cRow3+j:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow3+j+lanes:][0]))
			vC.Add(acc31).StorePtr(unsafe.Pointer(&c[cRow3+j+lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow3+j+2*lanes:][0]))
			vC.Add(acc32).StorePtr(unsafe.Pointer(&c[cRow3+j+2*lanes:][0]))
			vC = asm.LoadBFloat16x8AVX2Ptr(unsafe.Pointer(&c[cRow3+j+3*lanes:][0]))
			vC.Add(acc33).StorePtr(unsafe.Pointer(&c[cRow3+j+3*lanes:][0]))
		}
		for ; j+lanes <= blockDim; j += lanes {
			acc0 := asm.ZeroBFloat16x8AVX2()
