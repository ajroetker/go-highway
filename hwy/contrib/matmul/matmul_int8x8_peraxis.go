...
//go:generate go run ../../../cmd/hwygen -input matmul_int8x8_peraxis.go -dispatch int8x8matmulperaxis -output . -targets avx2,avx512,neon:asm,fallback

import "github.com/ajroetker/go-highway/hwy"

// BaseInt8x8MatMulPerAxis performs integer matrix multiplication of two uint8 matrices
// with per-axis zero-point subtraction, accumulating into int32.
//
// output[m,n] = sum_k( (int32(a[m,k]) - int32(aZP[m])) * (int32(b[k,n]) - int32(bZP[n])) )
//
// This supports ONNX MatMulInteger with per-axis (1D tensor) zero points.
// aZP has length M (one per row of A), bZP has length N (one per column of B).
//
// Parameters:
...
//   - aZP: per-row zero points for a (length M)
//   - bZP: per-column zero points for b (length N)
//   - M, K, N: matrix dimensions
func BaseInt8x8MatMulPerAxis(output []int32, a, b []uint8, aZP, bZP []uint8, M, K, N int) {
	if M == 0 || K == 0 || N == 0 {
		return
	}
