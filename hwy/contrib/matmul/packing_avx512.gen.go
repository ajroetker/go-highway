...
func BasePackLHSVec_avx512_Float16(a []hwy.Float16, packed []hwy.Float16, m int, k int, rowStart int, colStart int, panelRows int, panelK int, mr int) int {
	...
	for panel := 0; panel < fullPanels; panel++ {
		...
		for kk = 0; kk+lanes <= panelK; kk += lanes {
			r0 := asm.LoadFloat16x16AVX512Ptr(unsafe.Pointer(&a[row0+kk:][0]))
			r1 := asm.LoadFloat16x16AVX512Ptr(unsafe.Pointer(&a[row1+kk:][0]))
			r2 := asm.LoadFloat16x16AVX512Ptr(unsafe.Pointer(&a[row2+kk:][0]))
			r3 := asm.LoadFloat16x16AVX512Ptr(unsafe.Pointer(&a[row3+kk:][0]))
...
func BasePackLHSVec_avx512_BFloat16(a []hwy.BFloat16, packed []hwy.BFloat16, m int, k int, rowStart int, colStart int, panelRows int, panelK int, mr int) int {
	...
	for panel := 0; panel < fullPanels; panel++ {
		...
		var kk int
		for kk = 0; kk+lanes <= panelK; kk += lanes {
			r0 := asm.LoadBFloat16x16AVX512Ptr(unsafe.Pointer(&a[row0+kk:][0]))
			r1 := asm.LoadBFloat16x16AVX512Ptr(unsafe.Pointer(&a[row1+kk:][0]))
			r2 := asm.LoadBFloat16x16AVX512Ptr(unsafe.Pointer(&a[row2+kk:][0]))
			r3 := asm.LoadBFloat16x16AVX512Ptr(unsafe.Pointer(&a[row3+kk:][0]))
			t0 := r0.InterleaveLower(r2)
			t2 := r0.InterleaveUpper(r2)
			t1 := r1.InterleaveLower(r3)
			t3 := r1.InterleaveUpper(r3)
...
func BasePackLHSVec_avx512(a []float32, packed []float32, m int, k int, rowStart int, colStart int, panelRows int, panelK int, mr int) int {
	...
	for panel := 0; panel < fullPanels; panel++ {
		...
		var kk int
		for kk = 0; kk+lanes <= panelK; kk += lanes {
			r0 := archsimd.LoadFloat32x16((*[16]float32)(unsafe.Pointer(&a[row0+kk])))
			r1 := archsimd.LoadFloat32x16((*[16]float32)(unsafe.Pointer(&a[row1+kk])))
			r2 := archsimd.LoadFloat32x16((*[16]float32)(unsafe.Pointer(&a[row2+kk])))
			r3 := archsimd.LoadFloat32x16((*[16]float32)(unsafe.Pointer(&a[row3+kk])))
			...
			c0.Store((*[16]float32)(unsafe.Pointer(&packed[packIdx])))
			c1.Store((*[16]float32)(unsafe.Pointer(&packed[packIdx+lanes])))
			c2.Store((*[16]float32)(unsafe.Pointer(&packed[packIdx+2*lanes])))
			c3.Store((*[16]float32)(unsafe.Pointer(&packed[packIdx+3*lanes])))
			packIdx += lanes * mr
		}
		for ; kk < panelK; kk++ {
...
func BasePackLHSVec_avx512_Float64(a []float64, packed []float64, m int, k int, rowStart int, colStart int, panelRows int, panelK int, mr int) int {
	...
	for panel := 0; panel < fullPanels; panel++ {
		...
		var kk int
		for kk = 0; kk+lanes <= panelK; kk += lanes {
			r0 := archsimd.LoadFloat64x8((*[8]float64)(unsafe.Pointer(&a[row0+kk])))
			r1 := archsimd.LoadFloat64x8((*[8]float64)(unsafe.Pointer(&a[row1+kk])))
			r2 := archsimd.LoadFloat64x8((*[8]float64)(unsafe.Pointer(&a[row2+kk])))
			r3 := archsimd.LoadFloat64x8((*[8]float64)(unsafe.Pointer(&a[row3+kk])))
			...
			c0.Store((*[8]float64)(unsafe.Pointer(&packed[packIdx])))
			c1.Store((*[8]float64)(unsafe.Pointer(&packed[packIdx+lanes])))
			c2.Store((*[8]float64)(unsafe.Pointer(&packed[packIdx+2*lanes])))
			c3.Store((*[8]float64)(unsafe.Pointer(&packed[packIdx+3*lanes])))
			packIdx += lanes * mr
		}
		for ; kk < panelK; kk++ {
			packed[packIdx] = a[row0+kk]
		...
		if validCols == nr && nr >= lanes && nr%lanes == 0 {
			for kk := range panelK {
				...
				for c := 0; c < nr; c += lanes {
					v := asm.LoadFloat16x16AVX512Ptr(unsafe.Pointer(&b[srcRow+baseCol+c:][0]))
	...
	for strip := 0; strip < panelCols; strip += nr {
		...
		if validCols == nr && nr >= lanes && nr%lanes == 0 {
			for kk := range panelK {
				srcRow := (rowStart + kk) * n
				for c := 0; c < nr; c += lanes {
					v := asm.LoadBFloat16x16AVX512Ptr(unsafe.Pointer(&b[srcRow+baseCol+c:][0]))
					v.StorePtr(unsafe.Pointer(&packed[dstIdx+c:][0]))
				}
				dstIdx += nr
			}
	...
	for strip := 0; strip < panelCols; strip += nr {
		...
		if validCols == nr && nr >= lanes && nr%lanes == 0 {
			for kk := range panelK {
				srcRow := (rowStart + kk) * n
				for c := 0; c < nr; c += lanes {
					v := archsimd.LoadFloat32x16((*[16]float32)(unsafe.Pointer(&b[srcRow+baseCol+c])))
	...
	for strip := 0; strip < panelCols; strip += nr {
		...
		if validCols == nr && nr >= lanes && nr%lanes == 0 {
			for kk := range panelK {
				srcRow := (rowStart + kk) * n
				for c := 0; c < nr; c += lanes {
					v := archsimd.LoadFloat64x8((*[8]float64)(unsafe.Pointer(&b[srcRow+baseCol+c])))
