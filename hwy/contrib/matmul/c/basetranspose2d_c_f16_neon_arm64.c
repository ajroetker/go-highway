...
// BaseTranspose2D for NEON hwy.Float16
...
#ifndef GOAT_PARSER
static void transposeBlockSIMD(float16_t *src, float16_t *dst, long startI, long startJ, long m, long k, long lanes) {
    ...
    for (long c = 0; c < lanes; c++) {
        vst1q_f16((float16_t*)(dst + (startJ + c) * m + startI), rows[c]);
    }
}
#endif

#ifndef GOAT_PARSER
static void transposeEdgesScalar(float16_t *src, long m, long k, float16_t *dst, long lanes) {
    long blockM = (m / lanes) * lanes;
...
void transpose2d_c_f16_neon(float16_t *src, long *pm, long *pk, float16_t *dst, long *plen_src, long *plen_dst) {
    long k = *pk;
    long len_dst = *plen_dst;
    long len_src = *plen_src;
    long m = *pm;
    if (len_src < m * k || len_dst < k * m) {
        return;
    }
    long lanes = 8;
    #pragma clang loop vectorize(disable) interleave(disable)
    for (long i = 0; i <= m - lanes; i += lanes) {
        #pragma clang loop vectorize(disable) interleave(disable)
