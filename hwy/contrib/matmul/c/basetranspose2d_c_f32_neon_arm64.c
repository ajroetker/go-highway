...
// BaseTranspose2D for NEON float32
...
#ifndef GOAT_PARSER
static void transposeBlockSIMD(float *src, float *dst, long startI, long startJ, long m, long k, long lanes) {
    ...
    for (long c = 0; c < lanes; c++) {
        vst1q_f32(dst + (startJ + c) * m + startI, rows[c]);
    }
}
#endif

#ifndef GOAT_PARSER
static void transposeEdgesScalar(float *src, long m, long k, float *dst, long lanes) {
...
void transpose2d_c_f32_neon(float *src, long *pm, long *pk, float *dst, long *plen_src, long *plen_dst) {
    long k = *pk;
