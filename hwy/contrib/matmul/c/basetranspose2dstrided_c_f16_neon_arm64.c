...
// BaseTranspose2DStrided for NEON hwy.Float16
...
#ifndef GOAT_PARSER
static void transposeBlockSIMDStrided(float16_t *src, float16_t *dst, long startI, long startJ, long k, long dstM, long lanes) {
    float16x8_t rows[lanes];
    #pragma clang loop vectorize(disable) interleave(disable)
    for (long r = 0; r < lanes; r++) {
        rows[r] = vld1q_f16((float16_t*)(src + (startI + r) * k + startJ));
    }
    #pragma clang loop vectorize(disable) interleave(disable)
    for (long stride = lanes / 2; stride >= 1; ) {
        float16x8_t newRows[lanes];
        #pragma clang loop vectorize(disable) interleave(disable)
        for (long i = 0; i < lanes; i += 2 * stride) {
            #pragma clang loop vectorize(disable) interleave(disable)
            for (long j = 0; j < stride; j++) {
                newRows[i + j] = vzip1q_f16(rows[i + j], rows[i + j + stride]);
                newRows[i + j + stride] = vzip2q_f16(rows[i + j], rows[i + j + stride]);
            }
        }
    ...
    for (long c = 0; c < lanes; c++) {
        vst1q_f16((float16_t*)(dst + (startJ + c) * dstM + startI), rows[c]);
    }
}
#endif

#ifndef GOAT_PARSER
static void transposeEdgesScalarStrided(float16_t *src, long rowStart, long rowEnd, long k, long dstM, float16_t *dst, long lanes) {
...
void transpose2dstrided_c_f16_neon(float16_t *src, long *prowStart, long *prowEnd, long *pk, long *pdstM, float16_t *dst, long *plen_src, long *plen_dst) {
    long dstM = *pdstM;
    long k = *pk;
