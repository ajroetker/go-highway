...
// BaseMatMul for NEON hwy.Float16
...
void matmul_c_f16_neon(float16_t *a, float16_t *b, float16_t *c, long *pm, long *pn, long *pk, long *plen_a, long *plen_b, long *plen_c) {
    long k = *pk;
    long len_a = *plen_a;
    long len_b = *plen_b;
    long len_c = *plen_c;
    ...
    for (long i = 0; i < m; i++) {
        float16_t *cRow = c + i * n;
        float16x8_t vZero = vdupq_n_f16(0.0);
        long lanes = 8;
        long j = 0;
        #pragma clang loop vectorize(disable) interleave(disable)
        for (j = 0; j + lanes <= n; j += lanes) {
            vst1q_f16((float16_t*)(cRow + j), vZero);
        }
        #pragma clang loop vectorize(disable) interleave(disable)
        for (; j < n; j++) {
            cRow[j] = 0;
        ...
        #pragma clang loop vectorize(disable) interleave(disable)
        for (long p = 0; p < k; p++) {
            float16_t aip = a[i * k + p];
            float16x8_t vA = vdupq_n_f16(aip);
            float16_t *bRow = b + p * n;
            #pragma clang loop vectorize(disable) interleave(disable)
            for (j = 0; j + lanes <= n; j += lanes) {
                float16x8_t vB = vld1q_f16((float16_t*)(bRow + j));
                float16x8_t vC = vld1q_f16((float16_t*)(cRow + j));
                vC = vfmaq_f16(vC, vA, vB);
                vst1q_f16((float16_t*)(cRow + j), vC);
            }
            #pragma clang loop vectorize(disable) interleave(disable)
            for (; j < n; j++) {
                cRow[j] += aip * bRow[j];
