...
// BaseTranspose2DStrided for NEON float64
...
#ifndef GOAT_PARSER
static void transposeBlockSIMDStrided(double *src, double *dst, long startI, long startJ, long k, long dstM, long lanes) {
    float64x2_t rows[lanes];
    #pragma clang loop vectorize(disable) interleave(disable)
    for (long r = 0; r < lanes; r++) {
        rows[r] = vld1q_f64(src + (startI + r) * k + startJ);
    }
    #pragma clang loop vectorize(disable) interleave(disable)
    for (long stride = lanes / 2; stride >= 1; ) {
        float64x2_t newRows[lanes];
        #pragma clang loop vectorize(disable) interleave(disable)
        for (long i = 0; i < lanes; i += 2 * stride) {
            #pragma clang loop vectorize(disable) interleave(disable)
            for (long j = 0; j < stride; j++) {
                newRows[i + j] = vzip1q_f64(rows[i + j], rows[i + j + stride]);
                newRows[i + j + stride] = vzip2q_f64(rows[i + j], rows[i + j + stride]);
            }
        }
    ...
    for (long c = 0; c < lanes; c++) {
        vst1q_f64(dst + (startJ + c) * dstM + startI, rows[c]);
    }
}
#endif

#ifndef GOAT_PARSER
static void transposeEdgesScalarStrided(double *src, long rowStart, long rowEnd, long k, long dstM, double *dst, long lanes) {
...
void transpose2dstrided_c_f64_neon(double *src, long *prowStart, long *prowEnd, long *pk, long *pdstM, double *dst, long *plen_src, long *plen_dst) {
    long dstM = *pdstM;
    long k = *pk;
