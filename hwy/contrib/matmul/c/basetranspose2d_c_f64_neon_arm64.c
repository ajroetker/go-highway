...
// BaseTranspose2D for NEON float64
...
#ifndef GOAT_PARSER
static void transposeBlockSIMD(double *src, double *dst, long startI, long startJ, long m, long k, long lanes) {
    ...
    for (long c = 0; c < lanes; c++) {
        vst1q_f64(dst + (startJ + c) * m + startI, rows[c]);
    }
}
#endif

#ifndef GOAT_PARSER
static void transposeEdgesScalar(double *src, long m, long k, double *dst, long lanes) {
...
void transpose2d_c_f64_neon(double *src, long *pm, long *pk, double *dst, long *plen_src, long *plen_dst) {
    long k = *pk;
