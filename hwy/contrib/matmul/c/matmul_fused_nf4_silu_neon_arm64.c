...
// NEON Fused NF4 Dequant + Matrix Multiplication + SiLU Activation for ARM64
// Compile with: -march=armv8-a+simd
//
// Performs fused dequantization, matmul, and SiLU activation in a single pass:
...
// =============================================================================
// fused_nf4_silu_matmul_neon: Fused NF4 dequant + matmul + SiLU using NEON
// =============================================================================
// Computes output = SiLU(input @ dequant(packed, scales))
//
// Parameters:
...
//
// func fused_nf4_silu_matmul_neon(input, packed, scales, output unsafe.Pointer,
//                                  M, K, N, groupSize, numGroups *int64)
void fused_nf4_silu_matmul_neon(float *input, unsigned char *packed, float *scales,
    ...
    // Exp constants for sigmoid (matching Go hwy constants)
    ...
    for (long m = 0; m < M; m++) {
        ...
        for (long n = 0; n < N; n += 4) {
            ...
            // Compute exp(-acc)
            float32x4_t neg_x = vnegq_f32(acc);
            ...
            // SiLU = x * sigmoid(x)
            float32x4_t result = vmulq_f32(acc, sigmoid);
...
// =============================================================================
// fused_int4_silu_matmul_neon: Fused Int4 dequant + matmul + SiLU using NEON
// =============================================================================
// Same as NF4 but uses symmetric integer quantization:
// Values 0-15 map to -8 to +7 (subtract 8)
//
// func fused_int4_silu_matmul_neon(input, packed, scales, output unsafe.Pointer,
//                                   M, K, N, groupSize, numGroups *int64)
void fused_int4_silu_matmul_neon(float *input, unsigned char *packed, float *scales,
                                  float *output, long *pM, long *pK, long *pN,
                                  long *pGroupSize, long *pNumGroups) {
    long M = *pM;
    long K = *pK;
