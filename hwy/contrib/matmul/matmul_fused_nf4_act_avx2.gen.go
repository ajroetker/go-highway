// Code generated by github.com/ajroetker/go-highway/cmd/hwygen. DO NOT EDIT.

//go:build amd64 && goexperiment.simd

package matmul

import (
	stdmath "math"
	"simd/archsimd"
	"sync"
	"unsafe"

	"github.com/ajroetker/go-highway/hwy/contrib/math"
)

// Hoisted constants - lazily initialized on first use to avoid init-time crashes
var (
	BaseFusedInt4MatMulGELUApprox_AVX2_coeff_f32 archsimd.Float32x8
	BaseFusedInt4MatMulGELU_AVX2_half_f32        archsimd.Float32x8
	BaseFusedInt4MatMulGELU_AVX2_invSqrt2_f32    archsimd.Float32x8
	BaseFusedInt4MatMulGELU_AVX2_one_f32         archsimd.Float32x8
	BaseFusedNF4MatMulGELUApprox_AVX2_coeff_f32  archsimd.Float32x8
	BaseFusedNF4MatMulGELU_AVX2_half_f32         archsimd.Float32x8
	BaseFusedNF4MatMulGELU_AVX2_invSqrt2_f32     archsimd.Float32x8
	BaseFusedNF4MatMulGELU_AVX2_one_f32          archsimd.Float32x8
	_matmulFusedNf4ActAVX2HoistOnce              sync.Once
)

func _matmulFusedNf4ActAVX2InitHoistedConstants() {
	_matmulFusedNf4ActAVX2HoistOnce.Do(func() {
		BaseFusedInt4MatMulGELUApprox_AVX2_coeff_f32 = archsimd.BroadcastFloat32x8(float32(1.702))
		BaseFusedInt4MatMulGELU_AVX2_half_f32 = archsimd.BroadcastFloat32x8(float32(0.5))
		BaseFusedInt4MatMulGELU_AVX2_invSqrt2_f32 = archsimd.BroadcastFloat32x8(float32(0.7071067811865476))
		BaseFusedInt4MatMulGELU_AVX2_one_f32 = archsimd.BroadcastFloat32x8(float32(1.0))
		BaseFusedNF4MatMulGELUApprox_AVX2_coeff_f32 = archsimd.BroadcastFloat32x8(float32(1.702))
		BaseFusedNF4MatMulGELU_AVX2_half_f32 = archsimd.BroadcastFloat32x8(float32(0.5))
		BaseFusedNF4MatMulGELU_AVX2_invSqrt2_f32 = archsimd.BroadcastFloat32x8(float32(0.7071067811865476))
		BaseFusedNF4MatMulGELU_AVX2_one_f32 = archsimd.BroadcastFloat32x8(float32(1.0))
	})
}

func BaseFusedInt4MatMulGELU_avx2(input []float32, packed []uint8, scales []float32, bias []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	dequantBuf := make([]float32, tileN)
	accBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			accBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var unsignedVal int
					if weightIdx%2 == 0 {
						unsignedVal = int(packed[packedIdx] & 0x0F)
					} else {
						unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = float32(unsignedVal-8) * scale
				}
				w0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				w1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[lanes])))
				w2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[2*lanes])))
				w3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[3*lanes])))
				acc0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
				acc0 = inputVal.MulAdd(w0, acc0)
				acc1 = inputVal.MulAdd(w1, acc1)
				acc2 = inputVal.MulAdd(w2, acc2)
				acc3 = inputVal.MulAdd(w3, acc3)
				acc0.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var unsignedVal int
					if weightIdx%2 == 0 {
						unsignedVal = int(packed[packedIdx] & 0x0F)
					} else {
						unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = float32(unsignedVal-8) * scale
				}
				weights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc = inputVal.MulAdd(weights, acc)
				acc.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				var unsignedVal int
				if weightIdx%2 == 0 {
					unsignedVal = int(packed[packedIdx] & 0x0F)
				} else {
					unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
				}
				groupIdx := n / groupSize
				scale := scales[scaleBase+groupIdx]
				accBuf[n] += inputRow[k] * float32(unsignedVal-8) * scale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			if bias != nil {
				biasVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bias[n])))
				acc = acc.Add(biasVec)
			}
			invSqrt2 := BaseFusedInt4MatMulGELU_AVX2_invSqrt2_f32
			half := BaseFusedInt4MatMulGELU_AVX2_half_f32
			one := BaseFusedInt4MatMulGELU_AVX2_one_f32
			scaled := acc.Mul(invSqrt2)
			erfVal := math.BaseErfVec_avx2(scaled)
			acc = acc.Mul(half.Mul(one.Add(erfVal)))
			acc.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			sum := accBuf[n]
			if bias != nil {
				sum += bias[n]
			}
			outputRow[n] = sum * 0.5 * (1.0 + float32(stdmath.Erf(float64(sum)*0.7071067811865476)))
		}
	}
}

func BaseFusedInt4MatMulGELUApprox_avx2(input []float32, packed []uint8, scales []float32, bias []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	dequantBuf := make([]float32, tileN)
	accBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			accBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var unsignedVal int
					if weightIdx%2 == 0 {
						unsignedVal = int(packed[packedIdx] & 0x0F)
					} else {
						unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = float32(unsignedVal-8) * scale
				}
				w0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				w1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[lanes])))
				w2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[2*lanes])))
				w3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[3*lanes])))
				acc0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
				acc0 = inputVal.MulAdd(w0, acc0)
				acc1 = inputVal.MulAdd(w1, acc1)
				acc2 = inputVal.MulAdd(w2, acc2)
				acc3 = inputVal.MulAdd(w3, acc3)
				acc0.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var unsignedVal int
					if weightIdx%2 == 0 {
						unsignedVal = int(packed[packedIdx] & 0x0F)
					} else {
						unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = float32(unsignedVal-8) * scale
				}
				weights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc = inputVal.MulAdd(weights, acc)
				acc.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				var unsignedVal int
				if weightIdx%2 == 0 {
					unsignedVal = int(packed[packedIdx] & 0x0F)
				} else {
					unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
				}
				groupIdx := n / groupSize
				scale := scales[scaleBase+groupIdx]
				accBuf[n] += inputRow[k] * float32(unsignedVal-8) * scale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			if bias != nil {
				biasVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bias[n])))
				acc = acc.Add(biasVec)
			}
			coeff := BaseFusedInt4MatMulGELUApprox_AVX2_coeff_f32
			scaled := acc.Mul(coeff)
			sig := math.BaseSigmoidVec_avx2(scaled)
			acc = acc.Mul(sig)
			acc.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			sum := accBuf[n]
			if bias != nil {
				sum += bias[n]
			}
			outputRow[n] = sum / (1.0 + float32(stdmath.Exp(float64(-1.702*sum))))
		}
	}
}

func BaseFusedInt4MatMulReLU_avx2(input []float32, packed []uint8, scales []float32, bias []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	dequantBuf := make([]float32, tileN)
	accBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			accBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var unsignedVal int
					if weightIdx%2 == 0 {
						unsignedVal = int(packed[packedIdx] & 0x0F)
					} else {
						unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = float32(unsignedVal-8) * scale
				}
				w0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				w1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[lanes])))
				w2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[2*lanes])))
				w3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[3*lanes])))
				acc0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
				acc0 = inputVal.MulAdd(w0, acc0)
				acc1 = inputVal.MulAdd(w1, acc1)
				acc2 = inputVal.MulAdd(w2, acc2)
				acc3 = inputVal.MulAdd(w3, acc3)
				acc0.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var unsignedVal int
					if weightIdx%2 == 0 {
						unsignedVal = int(packed[packedIdx] & 0x0F)
					} else {
						unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = float32(unsignedVal-8) * scale
				}
				weights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc = inputVal.MulAdd(weights, acc)
				acc.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				var unsignedVal int
				if weightIdx%2 == 0 {
					unsignedVal = int(packed[packedIdx] & 0x0F)
				} else {
					unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
				}
				groupIdx := n / groupSize
				scale := scales[scaleBase+groupIdx]
				accBuf[n] += inputRow[k] * float32(unsignedVal-8) * scale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			if bias != nil {
				biasVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bias[n])))
				acc = acc.Add(biasVec)
			}
			acc = acc.Max(archsimd.BroadcastFloat32x8(0))
			acc.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			sum := accBuf[n]
			if bias != nil {
				sum += bias[n]
			}
			outputRow[n] = float32(stdmath.Max(0, float64(sum)))
		}
	}
}

func BaseFusedInt4MatMulSiLU_avx2(input []float32, packed []uint8, scales []float32, bias []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	dequantBuf := make([]float32, tileN)
	accBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			accBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var unsignedVal int
					if weightIdx%2 == 0 {
						unsignedVal = int(packed[packedIdx] & 0x0F)
					} else {
						unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = float32(unsignedVal-8) * scale
				}
				w0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				w1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[lanes])))
				w2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[2*lanes])))
				w3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[3*lanes])))
				acc0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
				acc0 = inputVal.MulAdd(w0, acc0)
				acc1 = inputVal.MulAdd(w1, acc1)
				acc2 = inputVal.MulAdd(w2, acc2)
				acc3 = inputVal.MulAdd(w3, acc3)
				acc0.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var unsignedVal int
					if weightIdx%2 == 0 {
						unsignedVal = int(packed[packedIdx] & 0x0F)
					} else {
						unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = float32(unsignedVal-8) * scale
				}
				weights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc = inputVal.MulAdd(weights, acc)
				acc.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				var unsignedVal int
				if weightIdx%2 == 0 {
					unsignedVal = int(packed[packedIdx] & 0x0F)
				} else {
					unsignedVal = int((packed[packedIdx] >> 4) & 0x0F)
				}
				groupIdx := n / groupSize
				scale := scales[scaleBase+groupIdx]
				accBuf[n] += inputRow[k] * float32(unsignedVal-8) * scale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			if bias != nil {
				biasVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bias[n])))
				acc = acc.Add(biasVec)
			}
			sig := math.BaseSigmoidVec_avx2(acc)
			acc = acc.Mul(sig)
			acc.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			sum := accBuf[n]
			if bias != nil {
				sum += bias[n]
			}
			outputRow[n] = sum / (1.0 + float32(stdmath.Exp(float64(-sum))))
		}
	}
}

func BaseFusedInt4MatMulSwiGLU_avx2(input []float32, gatePacked []uint8, gateScales []float32, upPacked []uint8, upScales []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	gateBuf := make([]float32, tileN)
	upBuf := make([]float32, tileN)
	gateAccBuf := make([]float32, N)
	upAccBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			gateAccBuf[i] = 0
			upAccBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					groupIdx := colIdx / groupSize
					var gateUnsigned int
					if weightIdx%2 == 0 {
						gateUnsigned = int(gatePacked[packedIdx] & 0x0F)
					} else {
						gateUnsigned = int((gatePacked[packedIdx] >> 4) & 0x0F)
					}
					gateScale := gateScales[scaleBase+groupIdx]
					gateBuf[lane] = float32(gateUnsigned-8) * gateScale
					var upUnsigned int
					if weightIdx%2 == 0 {
						upUnsigned = int(upPacked[packedIdx] & 0x0F)
					} else {
						upUnsigned = int((upPacked[packedIdx] >> 4) & 0x0F)
					}
					upScale := upScales[scaleBase+groupIdx]
					upBuf[lane] = float32(upUnsigned-8) * upScale
				}
				gw0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[0])))
				gw1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[lanes])))
				gw2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[2*lanes])))
				gw3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[3*lanes])))
				uw0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[0])))
				uw1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[lanes])))
				uw2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[2*lanes])))
				uw3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[3*lanes])))
				ga0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
				ga1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+lanes])))
				ga2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+2*lanes])))
				ga3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+3*lanes])))
				ua0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
				ua1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n+lanes])))
				ua2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n+2*lanes])))
				ua3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n+3*lanes])))
				ga0 = inputVal.MulAdd(gw0, ga0)
				ga1 = inputVal.MulAdd(gw1, ga1)
				ga2 = inputVal.MulAdd(gw2, ga2)
				ga3 = inputVal.MulAdd(gw3, ga3)
				ua0 = inputVal.MulAdd(uw0, ua0)
				ua1 = inputVal.MulAdd(uw1, ua1)
				ua2 = inputVal.MulAdd(uw2, ua2)
				ua3 = inputVal.MulAdd(uw3, ua3)
				ga0.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
				ga1.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+lanes])))
				ga2.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+2*lanes])))
				ga3.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+3*lanes])))
				ua0.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
				ua1.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n+lanes])))
				ua2.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n+2*lanes])))
				ua3.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					groupIdx := colIdx / groupSize
					var gateUnsigned int
					if weightIdx%2 == 0 {
						gateUnsigned = int(gatePacked[packedIdx] & 0x0F)
					} else {
						gateUnsigned = int((gatePacked[packedIdx] >> 4) & 0x0F)
					}
					gateScale := gateScales[scaleBase+groupIdx]
					gateBuf[lane] = float32(gateUnsigned-8) * gateScale
					var upUnsigned int
					if weightIdx%2 == 0 {
						upUnsigned = int(upPacked[packedIdx] & 0x0F)
					} else {
						upUnsigned = int((upPacked[packedIdx] >> 4) & 0x0F)
					}
					upScale := upScales[scaleBase+groupIdx]
					upBuf[lane] = float32(upUnsigned-8) * upScale
				}
				gateWeights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[0])))
				upWeights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[0])))
				gateAcc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
				upAcc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
				gateAcc = inputVal.MulAdd(gateWeights, gateAcc)
				upAcc = inputVal.MulAdd(upWeights, upAcc)
				gateAcc.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
				upAcc.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				groupIdx := n / groupSize
				var gateUnsigned int
				if weightIdx%2 == 0 {
					gateUnsigned = int(gatePacked[packedIdx] & 0x0F)
				} else {
					gateUnsigned = int((gatePacked[packedIdx] >> 4) & 0x0F)
				}
				gateScale := gateScales[scaleBase+groupIdx]
				gateAccBuf[n] += inputRow[k] * float32(gateUnsigned-8) * gateScale
				var upUnsigned int
				if weightIdx%2 == 0 {
					upUnsigned = int(upPacked[packedIdx] & 0x0F)
				} else {
					upUnsigned = int((upPacked[packedIdx] >> 4) & 0x0F)
				}
				upScale := upScales[scaleBase+groupIdx]
				upAccBuf[n] += inputRow[k] * float32(upUnsigned-8) * upScale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			gateAcc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
			upAcc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
			gateSilu := gateAcc.Mul(math.BaseSigmoidVec_avx2(gateAcc))
			result := gateSilu.Mul(upAcc)
			result.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			gateSum := gateAccBuf[n]
			upSum := upAccBuf[n]
			gateSilu := gateSum / (1.0 + float32(stdmath.Exp(float64(-gateSum))))
			outputRow[n] = gateSilu * upSum
		}
	}
}

func BaseFusedNF4MatMulGELU_avx2(input []float32, packed []uint8, scales []float32, bias []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	dequantBuf := make([]float32, tileN)
	accBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			accBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var quantIdx int
					if weightIdx%2 == 0 {
						quantIdx = int(packed[packedIdx] & 0x0F)
					} else {
						quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = nf4LookupTable[quantIdx] * scale
				}
				w0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				w1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[lanes])))
				w2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[2*lanes])))
				w3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[3*lanes])))
				acc0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
				acc0 = inputVal.MulAdd(w0, acc0)
				acc1 = inputVal.MulAdd(w1, acc1)
				acc2 = inputVal.MulAdd(w2, acc2)
				acc3 = inputVal.MulAdd(w3, acc3)
				acc0.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var quantIdx int
					if weightIdx%2 == 0 {
						quantIdx = int(packed[packedIdx] & 0x0F)
					} else {
						quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = nf4LookupTable[quantIdx] * scale
				}
				weights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc = inputVal.MulAdd(weights, acc)
				acc.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				var quantIdx int
				if weightIdx%2 == 0 {
					quantIdx = int(packed[packedIdx] & 0x0F)
				} else {
					quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
				}
				groupIdx := n / groupSize
				scale := scales[scaleBase+groupIdx]
				accBuf[n] += inputRow[k] * nf4LookupTable[quantIdx] * scale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			if bias != nil {
				biasVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bias[n])))
				acc = acc.Add(biasVec)
			}
			invSqrt2 := BaseFusedNF4MatMulGELU_AVX2_invSqrt2_f32
			half := BaseFusedNF4MatMulGELU_AVX2_half_f32
			one := BaseFusedNF4MatMulGELU_AVX2_one_f32
			scaled := acc.Mul(invSqrt2)
			erfVal := math.BaseErfVec_avx2(scaled)
			acc = acc.Mul(half.Mul(one.Add(erfVal)))
			acc.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			sum := accBuf[n]
			if bias != nil {
				sum += bias[n]
			}
			outputRow[n] = sum * 0.5 * (1.0 + float32(stdmath.Erf(float64(sum)*0.7071067811865476)))
		}
	}
}

func BaseFusedNF4MatMulGELUApprox_avx2(input []float32, packed []uint8, scales []float32, bias []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	dequantBuf := make([]float32, tileN)
	accBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			accBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var quantIdx int
					if weightIdx%2 == 0 {
						quantIdx = int(packed[packedIdx] & 0x0F)
					} else {
						quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = nf4LookupTable[quantIdx] * scale
				}
				w0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				w1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[lanes])))
				w2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[2*lanes])))
				w3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[3*lanes])))
				acc0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
				acc0 = inputVal.MulAdd(w0, acc0)
				acc1 = inputVal.MulAdd(w1, acc1)
				acc2 = inputVal.MulAdd(w2, acc2)
				acc3 = inputVal.MulAdd(w3, acc3)
				acc0.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var quantIdx int
					if weightIdx%2 == 0 {
						quantIdx = int(packed[packedIdx] & 0x0F)
					} else {
						quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = nf4LookupTable[quantIdx] * scale
				}
				weights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc = inputVal.MulAdd(weights, acc)
				acc.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				var quantIdx int
				if weightIdx%2 == 0 {
					quantIdx = int(packed[packedIdx] & 0x0F)
				} else {
					quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
				}
				groupIdx := n / groupSize
				scale := scales[scaleBase+groupIdx]
				accBuf[n] += inputRow[k] * nf4LookupTable[quantIdx] * scale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			if bias != nil {
				biasVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bias[n])))
				acc = acc.Add(biasVec)
			}
			coeff := BaseFusedNF4MatMulGELUApprox_AVX2_coeff_f32
			scaled := acc.Mul(coeff)
			sig := math.BaseSigmoidVec_avx2(scaled)
			acc = acc.Mul(sig)
			acc.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			sum := accBuf[n]
			if bias != nil {
				sum += bias[n]
			}
			outputRow[n] = sum / (1.0 + float32(stdmath.Exp(float64(-1.702*sum))))
		}
	}
}

func BaseFusedNF4MatMulReLU_avx2(input []float32, packed []uint8, scales []float32, bias []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	dequantBuf := make([]float32, tileN)
	accBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			accBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var quantIdx int
					if weightIdx%2 == 0 {
						quantIdx = int(packed[packedIdx] & 0x0F)
					} else {
						quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = nf4LookupTable[quantIdx] * scale
				}
				w0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				w1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[lanes])))
				w2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[2*lanes])))
				w3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[3*lanes])))
				acc0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
				acc0 = inputVal.MulAdd(w0, acc0)
				acc1 = inputVal.MulAdd(w1, acc1)
				acc2 = inputVal.MulAdd(w2, acc2)
				acc3 = inputVal.MulAdd(w3, acc3)
				acc0.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var quantIdx int
					if weightIdx%2 == 0 {
						quantIdx = int(packed[packedIdx] & 0x0F)
					} else {
						quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = nf4LookupTable[quantIdx] * scale
				}
				weights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc = inputVal.MulAdd(weights, acc)
				acc.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				var quantIdx int
				if weightIdx%2 == 0 {
					quantIdx = int(packed[packedIdx] & 0x0F)
				} else {
					quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
				}
				groupIdx := n / groupSize
				scale := scales[scaleBase+groupIdx]
				accBuf[n] += inputRow[k] * nf4LookupTable[quantIdx] * scale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			if bias != nil {
				biasVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bias[n])))
				acc = acc.Add(biasVec)
			}
			acc = acc.Max(archsimd.BroadcastFloat32x8(0))
			acc.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			sum := accBuf[n]
			if bias != nil {
				sum += bias[n]
			}
			outputRow[n] = float32(stdmath.Max(0, float64(sum)))
		}
	}
}

func BaseFusedNF4MatMulSiLU_avx2(input []float32, packed []uint8, scales []float32, bias []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	dequantBuf := make([]float32, tileN)
	accBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			accBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var quantIdx int
					if weightIdx%2 == 0 {
						quantIdx = int(packed[packedIdx] & 0x0F)
					} else {
						quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = nf4LookupTable[quantIdx] * scale
				}
				w0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				w1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[lanes])))
				w2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[2*lanes])))
				w3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[3*lanes])))
				acc0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
				acc0 = inputVal.MulAdd(w0, acc0)
				acc1 = inputVal.MulAdd(w1, acc1)
				acc2 = inputVal.MulAdd(w2, acc2)
				acc3 = inputVal.MulAdd(w3, acc3)
				acc0.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc1.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+lanes])))
				acc2.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+2*lanes])))
				acc3.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					var quantIdx int
					if weightIdx%2 == 0 {
						quantIdx = int(packed[packedIdx] & 0x0F)
					} else {
						quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
					}
					groupIdx := colIdx / groupSize
					scale := scales[scaleBase+groupIdx]
					dequantBuf[lane] = nf4LookupTable[quantIdx] * scale
				}
				weights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&dequantBuf[0])))
				acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
				acc = inputVal.MulAdd(weights, acc)
				acc.StoreArray((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				var quantIdx int
				if weightIdx%2 == 0 {
					quantIdx = int(packed[packedIdx] & 0x0F)
				} else {
					quantIdx = int((packed[packedIdx] >> 4) & 0x0F)
				}
				groupIdx := n / groupSize
				scale := scales[scaleBase+groupIdx]
				accBuf[n] += inputRow[k] * nf4LookupTable[quantIdx] * scale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			acc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&accBuf[n])))
			if bias != nil {
				biasVec := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&bias[n])))
				acc = acc.Add(biasVec)
			}
			sig := math.BaseSigmoidVec_avx2(acc)
			acc = acc.Mul(sig)
			acc.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			sum := accBuf[n]
			if bias != nil {
				sum += bias[n]
			}
			outputRow[n] = sum / (1.0 + float32(stdmath.Exp(float64(-sum))))
		}
	}
}

func BaseFusedNF4MatMulSwiGLU_avx2(input []float32, gatePacked []uint8, gateScales []float32, upPacked []uint8, upScales []float32, output []float32, M int, K int, N int, groupSize int) {
	_matmulFusedNf4ActAVX2InitHoistedConstants()
	if M == 0 || K == 0 || N == 0 {
		return
	}
	numGroups := (N + groupSize - 1) / groupSize
	lanes := 8
	tileN := 4 * lanes
	gateBuf := make([]float32, tileN)
	upBuf := make([]float32, tileN)
	gateAccBuf := make([]float32, N)
	upAccBuf := make([]float32, N)
	for m := range M {
		inputRow := input[m*K : (m+1)*K]
		outputRow := output[m*N : (m+1)*N]
		for i := range N {
			gateAccBuf[i] = 0
			upAccBuf[i] = 0
		}
		for k := range K {
			inputVal := archsimd.BroadcastFloat32x8(inputRow[k])
			baseIdx := k * N
			scaleBase := k * numGroups
			var n int
			for n = 0; n+tileN <= N; n += tileN {
				for lane := range tileN {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					groupIdx := colIdx / groupSize
					var gateQuantIdx int
					if weightIdx%2 == 0 {
						gateQuantIdx = int(gatePacked[packedIdx] & 0x0F)
					} else {
						gateQuantIdx = int((gatePacked[packedIdx] >> 4) & 0x0F)
					}
					gateScale := gateScales[scaleBase+groupIdx]
					gateBuf[lane] = nf4LookupTable[gateQuantIdx] * gateScale
					var upQuantIdx int
					if weightIdx%2 == 0 {
						upQuantIdx = int(upPacked[packedIdx] & 0x0F)
					} else {
						upQuantIdx = int((upPacked[packedIdx] >> 4) & 0x0F)
					}
					upScale := upScales[scaleBase+groupIdx]
					upBuf[lane] = nf4LookupTable[upQuantIdx] * upScale
				}
				gw0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[0])))
				gw1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[lanes])))
				gw2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[2*lanes])))
				gw3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[3*lanes])))
				uw0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[0])))
				uw1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[lanes])))
				uw2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[2*lanes])))
				uw3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[3*lanes])))
				ga0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
				ga1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+lanes])))
				ga2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+2*lanes])))
				ga3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+3*lanes])))
				ua0 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
				ua1 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n+lanes])))
				ua2 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n+2*lanes])))
				ua3 := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n+3*lanes])))
				ga0 = inputVal.MulAdd(gw0, ga0)
				ga1 = inputVal.MulAdd(gw1, ga1)
				ga2 = inputVal.MulAdd(gw2, ga2)
				ga3 = inputVal.MulAdd(gw3, ga3)
				ua0 = inputVal.MulAdd(uw0, ua0)
				ua1 = inputVal.MulAdd(uw1, ua1)
				ua2 = inputVal.MulAdd(uw2, ua2)
				ua3 = inputVal.MulAdd(uw3, ua3)
				ga0.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
				ga1.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+lanes])))
				ga2.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+2*lanes])))
				ga3.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n+3*lanes])))
				ua0.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
				ua1.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n+lanes])))
				ua2.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n+2*lanes])))
				ua3.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n+3*lanes])))
			}
			for ; n+lanes <= N; n += lanes {
				for lane := range lanes {
					colIdx := n + lane
					weightIdx := baseIdx + colIdx
					packedIdx := weightIdx / 2
					groupIdx := colIdx / groupSize
					var gateQuantIdx int
					if weightIdx%2 == 0 {
						gateQuantIdx = int(gatePacked[packedIdx] & 0x0F)
					} else {
						gateQuantIdx = int((gatePacked[packedIdx] >> 4) & 0x0F)
					}
					gateScale := gateScales[scaleBase+groupIdx]
					gateBuf[lane] = nf4LookupTable[gateQuantIdx] * gateScale
					var upQuantIdx int
					if weightIdx%2 == 0 {
						upQuantIdx = int(upPacked[packedIdx] & 0x0F)
					} else {
						upQuantIdx = int((upPacked[packedIdx] >> 4) & 0x0F)
					}
					upScale := upScales[scaleBase+groupIdx]
					upBuf[lane] = nf4LookupTable[upQuantIdx] * upScale
				}
				gateWeights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateBuf[0])))
				upWeights := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upBuf[0])))
				gateAcc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
				upAcc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
				gateAcc = inputVal.MulAdd(gateWeights, gateAcc)
				upAcc = inputVal.MulAdd(upWeights, upAcc)
				gateAcc.StoreArray((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
				upAcc.StoreArray((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
			}
			for ; n < N; n++ {
				weightIdx := baseIdx + n
				packedIdx := weightIdx / 2
				groupIdx := n / groupSize
				var gateQuantIdx int
				if weightIdx%2 == 0 {
					gateQuantIdx = int(gatePacked[packedIdx] & 0x0F)
				} else {
					gateQuantIdx = int((gatePacked[packedIdx] >> 4) & 0x0F)
				}
				gateScale := gateScales[scaleBase+groupIdx]
				gateAccBuf[n] += inputRow[k] * nf4LookupTable[gateQuantIdx] * gateScale
				var upQuantIdx int
				if weightIdx%2 == 0 {
					upQuantIdx = int(upPacked[packedIdx] & 0x0F)
				} else {
					upQuantIdx = int((upPacked[packedIdx] >> 4) & 0x0F)
				}
				upScale := upScales[scaleBase+groupIdx]
				upAccBuf[n] += inputRow[k] * nf4LookupTable[upQuantIdx] * upScale
			}
		}
		var n int
		for n = 0; n+lanes <= N; n += lanes {
			gateAcc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&gateAccBuf[n])))
			upAcc := archsimd.LoadFloat32x8Array((*[8]float32)(unsafe.Pointer(&upAccBuf[n])))
			gateSilu := gateAcc.Mul(math.BaseSigmoidVec_avx2(gateAcc))
			result := gateSilu.Mul(upAcc)
			result.StoreArray((*[8]float32)(unsafe.Pointer(&outputRow[n])))
		}
		for ; n < N; n++ {
			gateSum := gateAccBuf[n]
			upSum := upAccBuf[n]
			gateSilu := gateSum / (1.0 + float32(stdmath.Exp(float64(-gateSum))))
			outputRow[n] = gateSilu * upSum
		}
	}
}
